...
rst.Close
rst.Open "select 标题,作者 from bbs where id="&fid,conn
msg=msg&"<hr><form action=replynow.asp method=post id=form1 name=form1><input type=hidden value='"&bid&"' name=bid><input type=hidden name=fid value="&Request.QueryString("fid")&"><table width=60% border=3 bgcolor=cccccc align=center><tr align=center><td colspan=2><font color=0000ff size=4> 回 复 贴 </font></td></tr><tr><td width='15%'>发贴人</td><td>"&username&"</td></tr><tr><td>●标题</td><td><input type=text name=title size=50 maxlength=50 value='RE:"&rst("标题")&"'></td></tr><tr><td colspan=2>●内容(<a href='ubbcode.htm' target='_blank'>UBB 代码</a>支持)</td></tr><tr><td colspan=2><textarea name=content wrap=PHYSICAL cols=65 rows=10>回复"&rst("作者")&"同志的大作:"&chr(13)&chr(10)&"</textarea></td></tr><tr align=center><td colspan=2><input type=submit value=' 发 表 ' id=submit1 name=submit1> <input type=reset value=' 重 填 ' id=reset1 name=reset1> <input type=button onclick='javascript:history.back();' value=' 返 回 ' id=button1 name=button1></td></tr></table></form>"&Ba_copyright&"</body></html>"
rst.Close 
set rst=nothing
conn.close
set conn=nothing
