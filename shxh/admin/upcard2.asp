...
msg="<head><link rel='stylesheet' href='../chatroom/css.css'></head><body oncontextmenu=self.event.returnValue=false topmargin=0 background='"&bgimage&"' bgcolor='"&bgcolor&"'><p align=center>道具管理</p><hr><table width=80% border=3 align=center><tr bgcolor=FFFF00><td width='35%'>属性</td><td>新值</td></tr>"
Set conn=Server.CreateObject("ADODB.CONNECTION")
conn.open Application("Ba_jxqy_connstr")
Set rst=Server.CreateObject("ADODB.RecordSet")
sqlstr="SELECT * FROM cardshop where id="&cid
rst.Open sqlstr,conn,1,2
if Request.Form("opt")="新增" then rst.AddNew
if rst.EOF or rst.BOF then Response.Redirect "../error.asp?id=101"
if Request.Form("opt")="删除" then 
	rst.Delete
	...
	for i=1 to rst.Fields.Count-1
		...
		elseif rst.Fields(i).type=130 then
			rst.Fields(i).Value=cstr(Request.Form(i+1))	
