...
if Application("yx8_mhjh_klt11")=0 then
...
tempjs=int(abs(clng(Application("yx8_mhjh_klt11"))))
'if tempjs<>r then
	'Response.Write "<script Language=Javascript>alert('提示：滚，你想搞什么，按回车键不行了!');</script>"
	'Response.Write "<script Language=Javascript>location.href = '../option.asp';</script>"
	'response.end
'end if
conn.execute "update 用户 set 体力=体力-"&Application("yx8_mhjh_klt11")&" where 姓名='"&st&"'"
...
conn.execute "insert into 英烈堂(时间,死者,凶手,死因) values(now(),'"&username&"','吝啬','体力不足累死')"
	...
	conn.execute "update 用户 set 防御=防御+"& Application("yx8_mhjh_klt11") *300&" where 姓名='" & st & "'"
	kl="<font color=#0000FF>"&st&"</font>一翻苦言,劝说大家不要伤害江湖保护动物,并把梦精灵送回山上，梦精灵帮<font color=#0000FF>"&st&"</font>提高防御"&Application("yx8_mhjh_klt11")*300&"点！"
	Application.Lock
	Application("yx8_mhjh_klt11")=0
	Application.UnLock
	rs.close
