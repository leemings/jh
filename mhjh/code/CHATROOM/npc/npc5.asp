...
if Application("yx8_mhjh_klt6")=0 then
...
'if tempjs<>r then
	...
	'response.end
'end if
conn.execute "update 用户 set 体力=体力-"&Application("yx8_mhjh_klt6")&" where 姓名='"&st&"'"
...
conn.execute "insert into 英烈堂(时间,死者,凶手,死因) values(now(),'"&username&"','吝啬','体力不足累死')"
	...
	conn.execute "update 用户 set 银两=银两+"& Application("yx8_mhjh_klt6") *100000&" where 姓名='" & st & "'"
	kl="<font color=#0000FF>"&st&"</font>一拳打死怪物,获得当地官府奖励白银"&Application("yx8_mhjh_klt6")*100000&"两！"
	Application.Lock
	Application("yx8_mhjh_klt6")=0
	Application.UnLock
	rs.close
