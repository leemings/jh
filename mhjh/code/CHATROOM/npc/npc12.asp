...
if Application("yx8_mhjh_klt13")=0 then
...
tempjs=int(abs(clng(Application("yx8_mhjh_klt13"))))
'if tempjs<>r then
	'Response.Write "<script Language=Javascript>alert('提示：滚，你想搞什么，按回车键不行了!');</script>"
	'Response.Write "<script Language=Javascript>location.href = '../option.asp';</script>"
	'response.end
'end if
conn.execute "update 用户 set 体力=体力-"&Application("yx8_mhjh_klt13")&" where 姓名='"&st&"'"
...
conn.execute "insert into 英烈堂(时间,死者,凶手,死因) values(now(),'"&username&"','吝啬','体力不足累死')"
	...
	conn.execute "update 用户 set 银两=银两+"& Application("yx8_mhjh_klt13") *20000&" where 姓名='" & st & "'"
	kl="<font color=#0000FF>"&st&"</font>成功把刺球宝宝赶走,并把走入魔道大侠挽救回来，获得了走火入魔的大侠身上"&Application("yx8_mhjh_klt13")*20000&"白银！"
	Application.Lock
	Application("yx8_mhjh_klt13")=0
	Application.UnLock
	rs.close
