...
if Application("yx8_mhjh_klt12")=0 then
...
tempjs=int(abs(clng(Application("yx8_mhjh_klt12"))))
'if tempjs<>r then
	'Response.Write "<script Language=Javascript>alert('提示：滚，你想搞什么，按回车键不行了!');</script>"
	'Response.Write "<script Language=Javascript>location.href = '../option.asp';</script>"
	'response.end
'end if
conn.execute "update 用户 set 体力=体力-"&Application("yx8_mhjh_klt12")&" where 姓名='"&st&"'"
...
conn.execute "insert into 英烈堂(时间,死者,凶手,死因) values(now(),'"&username&"','吝啬','体力不足累死')"
	...
	conn.execute "update 用户 set 银两=银两+"& Application("yx8_mhjh_klt12") *10000&" where 姓名='" & st & "'"
	kl="<font color=#0000FF>"&st&"</font>成功消灭火史莱姆，夺取它身上的银两"&Application("yx8_mhjh_klt12")*10000&"两！"
	Application.Lock
	Application("yx8_mhjh_klt12")=0
	Application.UnLock
	rs.close
