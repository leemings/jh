...
if Application("yx8_mhjh_klt3")=0 then
...
tempjs=int(abs(clng(Application("yx8_mhjh_klt3"))))
'if tempjs<>jst3 then
	'Response.Write "<script Language=Javascript>alert('提示：滚，你想搞什么，按回车键不行了!');</script>"
	'Response.Write "<script Language=Javascript>location.href ='javascript:window.close()';</script>"
	'response.end
'end if
conn.execute "update 用户 set 体力=体力-"&Application("yx8_mhjh_klt3")&" where 姓名='"&st&"'"
...
conn.execute "insert into 英烈堂(时间,死者,凶手,死因) values(now(),'"&username&"','吝啬','体力不足累死')"
	...
	conn.execute "update 用户 set 体力=体力+"& Application("yx8_mhjh_klt3") *30000&",内力=内力+"&Application("yx8_mhjh_klt3")*10000&" where 姓名='" & st & "'"
	kl="<font color=0000FF>"&st&"</font>动作迅速，成功抢得武术家掉下的体力<img src=IMAGE/IMAGE/wg7.gif>"&Application("yx8_mhjh_klt3")*30000&"点，内力"&Application("yx8_mhjh_klt3")*10000&"点！"
	Application.Lock
	Application("yx8_mhjh_klt3")=0
	Application.UnLock
	rs.close
