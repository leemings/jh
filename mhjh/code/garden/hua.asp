...
rs.open "select 银两,体力 from 用户 where 姓名='"& sjjh_name &"'",conn
if rs("银两")<50000 then
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
Response.Write "<script language=JavaScript>{alert('请准备50000两占地费,不一定什么时候收！');window.close();}</script>"
Response.End
end if
if rs("体力")<50000 then
rs.close
...
conn.execute "update 用户 set 体力=体力-5 where 姓名='"& sjjh_name &"'"
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
...
Sub Msg (v)
 ...
 Response.End
End Sub
If session("yx8_mhjh_username")= "" Then Msg "您尚未登录，不能种花。"
name =session("yx8_mhjh_username")
If name = "" Then Msg "您尚未登录，不能种花。"
%>
<HTML><HEAD><TITLE>魔幻花园</TITLE>
<script>
window.moveTo(40,30);
</script>
...
function go(i,fc,ft,fa){
	//if(i>24||fc==''||fc==''||fc==''){
//	flowersact.innerHTML="江湖有问题，请刷新叶面";
//	return;
//	}
...
//alert("此处不存在鲜花无法操作!");return;
		...
		if(confirm("您要将花采下换为精力，还是将花变成3颗种子？\n点确定是采下换精力，点取消是换3颗种子。")) {
...
function count() {
   ...
   if (random>0.3&&random<0.4&&dialogcount>0)
	...
	if (random>=0.8&&random<0.88&&dialogcount>0)
		dialogbox.innerHTML="忘了留种子可以重新开垦一下，站长免费送三颗！";
...
set rs=server.CreateObject("adodb.recordset")
sjjh_grade=session("yx8_mhjh_usergrade")
rs.open "select 会员,hua from 用户 where 姓名='"&sjjh_name&"'",conn,2,2
if rs("hua")<>"" then
else
