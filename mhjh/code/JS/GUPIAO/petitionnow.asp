...
if num<1 then num=1
msg="<head><link rel='stylesheet' href='../../style.css'><script language=javascript>setTimeout('history.back();',3000);</script></head><body oncontextmenu=self.event.returnValue=false topmargin=0 bgcolor='#339966'><h3>申购原始股</h3><hr>"
...
rs1.Open "select 银两 from 用户 where 姓名='"&username&"'",conn1,2,2
...
umoney=num*uprice
rs.Close
rs.Open "select * from 持股 where 持股人='"&username&"' and 名称='"&stock&"'",conn,2,2
if rs.EOF or rs.BOF then
	...
	conn1.Execute "update 用户 set 银两=银两-"&umoney&" where 姓名='"&username&"'"
rs.Close
set rs=nothing
