...
if (f[iMove][jMove]!=0) {alert('不好意思，此处己被占用！'); return; }
...
if (winningPos(iMove,jMove,userSq)==winningMove) setTimeout("gameOver=1;alert('恭喜您赢到了500精力！'); parent.t.location.href='button1.asp';",dly);
...
if (winningPos(iMach,jMach,machSq)==winningMove) setTimeout('gameOver=1;alert("不好意思你输喽，再来一次吧!")',900);
else if (drawPos) setTimeout('gameOver=1;alert("呵呵，打成平手了，再来一局吧!")',900);
...
if (winningPos(iMach,jMach,machSq)==winningMove) setTimeout('gameOver=1;alert("对不起，您输了!")',900);
...
if (winningPos(iHint,jHint,userSq)==winningMove) setTimeout('gameOver=1;alert("对不起,您输了!")',900);
...
buf+='</head><body><a name="s"></a><center><pre';
for (i=0;i<boardSize;i++) {
for (j=0;j<boardSize;j++) {
