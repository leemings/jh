...
<meta name=keywords content="魔幻林江湖股票市场">
        	...
        	<tr valign=top bgcolor="#E4E8EF"> 
          		<td align=center width="100%"> 
					<table cellspacing=1 cellpadding=0 border=0 width="200">
						...
						<tr><td>综合指数：</td><td align=right><%if ColExp>0 then%><font color=#FF0000>↑<%elseif ColExp<0 then%><font color="#0066FF">↓<%else%><font color="#0066FF">→ <%end if%><%=formatnumber(abs(ColExp),2,true)%>%</font></td></tr>
						<tr><td colspan="2" height="3"></td></tr>
					</table>			
				</td>
			</tr>
		...
		<% else %>
			...
			<tr>
				<td width=250 bgcolor="#ffffff"> 
					<input type=hidden name=username value="<%=session("yx8_mhjh_username")%>">
	...
	<tr>  
		<td  height=32 background="images/footer.gif" valign=top>
			<table border=0 width="100%" cellspacing=0 cellpadding=5 bordercolorlight="#ffffff" bordercolordark="#ffffff">
				<tr valign="bottom"><td id="clock" width="185" align="left">欢迎你</td><td width="*" id="CopyRight" align="center">制作小组：yx8.net</td><td width="170" align="right">页面执行时间：<%=FormatNumber((timer()-startime)*1000,3)%>毫秒</td></tr>
			</table>	
		</td>
	</tr>
</table> 
