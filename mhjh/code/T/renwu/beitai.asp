...
nu=int(rnd*18)+1
s=int(rnd*100)
mess="北太行山的野狼果然厉害，你被群狼攻击，身负重伤。你的攻击和防御因此减少了"& s &""
...
nu=int(rnd*18)+1
s=int(rnd*100)
mess="你在北太行山遇到了山寨精英，被掠夺去了银子"& s &"两"
sql="update 用户 set 银两=银两-'"& s &"'  where 姓名='" &username& "'"
conn.execute sql
conn.close
elseif r=5 or r=6 then
mess="北太行山险道你见一剑客练剑，一打听原来是令狐冲，悄悄偷学了几招，资质增加了20"
sql="update 用户 set 资质=资质+20  where 姓名='" &username& "'"
conn.execute sql
conn.close
elseif r=7 or r=8 then
mess="经过与群狼战斗，虽然体力损失10000，但你得到了白狼皮一张，可以兑换唤醒值，以便将来唤醒NPC得到一些装备，或者进入特别的地区！"
sql="update 用户 set 任务='狼皮',体力=体力-10000  where 姓名='" &username& "'"
conn.execute sql
conn.close
else
mess="恭喜您!你来到北太行山山寨竟然没有遇到任何人，直接从山寨头子宝座上得到了风铃，可以兑换唤醒值，以便将来唤醒NPC得到一些装备，或者进入特别的地区！"
sql="update 用户 set 任务='风铃'  where 姓名='" &username& "'"
conn.execute sql
conn.close
end if
set conn=nothing
