...
var layerCanMove=new Array();

var HitTestList=new Array();



var curLayer=null;        //add by zhihong.d for sign current drag layer
...
var callbackDragBegin,callbackDragEnd;
//-- add by zhihong.d working for mouse event   2000/3/21

//-- if you want to let layer movable, you can do as...

//        (1)<BODY onload="layerEventInit();">

//  (2) createLayer(...);

//-- in the default status, after you do layerEventInit, the all layer can be moved.

//-- so you can use layerSetMove(...,...) to enable one layer's move ability,

//-- the layerCanMove array records your layers can move.

//-- the layerSetMove's first param is the layer's name, and the second param is the area's height

//-- that you want the layer to be draged over, the default value is -1, means the full area

//-- the JavaScript don't have object(rect), so this is a bug that we decide the area from (0,0)to(height,fullWidth)

//-- in the future, maybe solve the problem

function layerEventInit()

{
        ...
        if(ns4)document.captureEvents(Event.MOUSEDOWN|Event.MOUSEUP|Event.MOUSEMOVE);

}
function layerEventInitDrag(dragbegin,dragend)
{
        callbackDragBegin=dragbegin;
        callbackDragEnd=dragend;
        layerEventInit();
}
...

{
...

        {
                ...
                moveLayer(curLayer,x-dragClickX,y-dragClickY);
...

        {
...

                {
...

                        {
                          ...
                          if(name==layerCanMove[j])

                          {

                                  iHeight=HitTestList[j];
                ...
                if(iHeight==-1)iHeight=getHeight(name);

                if(x>=getLeft(name)&&x<=(getLeft(name)+getWidth(name))&&

                 y>=getTop(name)&&y<=getTop(name)+iHeight)

                {

                        bCapture=true;

                        dragClickX=x-getLeft(name);

                        dragClickY=y-getTop(name);

                        curLayer=name;

                        if(callbackDragBegin)
                        {
                                callbackDragBegin(name);
                        }
                        break;
...

{
        ...
        {
                bCapture=false;
                if(callbackDragEnd)callbackDragEnd(curLayer);
        }
}

function layerSetMove(name,iHeight)

{//--- if you set iHeight to -1, we use the full height

        var z=layerCanMove.length;

        layerCanMove[z]=name;

        if(iHeight==-1)iHeight=getHeight(name);

        HitTestList[z]=iHeight;
...
function MessageBox(layername,strTitle,content,width)
{
      var strReturn='<table bgcolor=cccccc border=0><tr><td>'+
	      '<table  bgcolor=ffc88f align=center cellspacing=0 cellpadding=0 border=0 width='+width+'>'+
          '<tr><td valign=top width=99% colspan=3>'+
          '<table cellspacing=0 cellpadding=0 border=0 width=95% align=center>'+
          '<tr><td class=ct-def2>'+strTitle+'</td>'+
          '<td align=right><a href=javascript:hideLayer('+layername+')><img src=http://img.the9.com/img/ico/no.gif width=12 height=12 alt=¹Ø±Õ border=0></a></td>'+
          '</tr></table></td></tr>'+
		  '<tr><td colspan=3 height=1 width=100% bgcolor=cccccc><img src=http://img.the9.com/img/empty.gif></td></tr>'+content+'</table></td></tr></table>';
      return strReturn;
}
//zhihong_d add end.
...
//        Add by Dennis Zou, 2000,3,28

function createTransLayerBmp(name, left, top, width, height, visible, trans,bmp,content) {
  ...
  if (document.all) {
    ...
    document.writeln('<div id="' + name + '" style="FILTER: alpha(opacity=' + trans + '); background-image: url('+bmp+'); position:absolute; overflow:none; left:' + left + 'px; top:' + top + 'px; width:' + width + 'px; height:' + height + 'px;' + ' visibility:' + (visible ? 'visible;' : 'hidden;') +  ' z-index:' + z + '">');
...
function createTransLayer(name, left, top, width, height, visible, trans, content) {
  ...
  if (document.all) {
    ...
    document.writeln('<div id="' + name + '" style="FILTER: alpha(opacity=' + trans + '); position:absolute; overflow:none; left:' + left + 'px; top:' + top + 'px; width:' + width + 'px; height:' + height + 'px;' + ' visibility:' + (visible ? 'visible;' : 'hidden;') +  ' z-index:' + z + '">');
...
function createLayerM2(name,left,top,width,height,visible,content,mDown,mUp)
{
  var z = layerList.length;
  var layer;
  layerList[z] = name;
  if (document.layers) {
    document.writeln('<layer name="' + name + '" onMouseDown="' + mDown + ';" onMouseUp="' + mUp + ';" left=' + left + ' top=' + top + ' width=' + width + ' height=' + height +  ' visibility=' + (visible ? '"show"' : '"hide"') + ' z-index=' + z + '>');
  ...
  if (document.all) {
    ...
    document.writeln('<div id="' + name + '" onMouseDown="' + mDown + ';" onMouseUp="' + mUp + ';" style="position:absolute; overflow:none; left:' + left + 'px; top:' + top + 'px; width:' + width + 'px; height:' + height + 'px;' + ' visibility:' + (visible ? 'visible;' : 'hidden;') + ' z-index:' + z + '">');
...
function time_goon(name)

{

//        scrollLayer(name,0,1);

moveByLayer(name,0,-3);

        setTimeout('time_goon("' + name + '")',50);

}

function goonLayer(name,iHeight)

{
  ...
  clipLayer(name,cl,-iHeight,cr,0);

  moveByLayer(name,0,iHeight);

//        movebyLayer(name,0,50);
