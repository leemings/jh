...
rs.open "SELECT id FROM w WHERE b="& sjjh_id & " and i>=15 and a='矿石'",conn
if rs.eof or rs.bof then 
	rs.close
	set rs=nothing
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('提示：你的矿石不够15个不能操作！');</script>"
	response.end
end if
kuangid=rs("id")
rs.close
...
iceid=rs("id")
rs.close
rs.open "SELECT id FROM w WHERE b=" & sjjh_id & " and i>=5 and a='大鲨鱼'",conn
if rs.eof or rs.bof then 
	rs.close
	set rs=nothing
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('提示：你的大鲨鱼不够5个不能操作！');</script>"
	response.end
end if
dsayu=rs("id")
rs.close
...
rs.close
rs.open "select id from x where a='舍利子' and b="&sjjh_id
If Rs.Bof OR Rs.Eof then
	conn.execute "insert into x(a,b,c,d,e) values ('舍利子'," & sjjh_id &",1,'杀人数',0)"
else
	id=rs("id")
