...
conn.execute "update 用户 set 银两=银两-10000 where 姓名='"& sjjh_name &"'"
conn.execute "update 用户 set 体力=体力-100 where 姓名='"& sjjh_name &"'"
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
...
Sub Msg (v)
 ...
 Response.End
End Sub
If Session("sjjh_name") = "" Then Msg "您尚未登录，不能种花。"
