...
'我赢了
if myzd>tozd then
	conn.execute "update 用户 set 内力=int(内力/2),银两=银两+"&int(toyin/2)&" where 姓名='" & sjjh_name &"'" &""
	conn.execute "update 用户 set 内力=100,体力=100,武功=100,银两=int(银两/2) where 姓名='"& name &"'"
	dantiao="二人战在一起，战得天昏地暗， {"& name &"} 终于不敌，落荒而逃......；["&sjjh_name&"]得意洋洋得说：这回知道我得厉害了吧！……得到银两："&int(toyin/2)
end if
'我输了
if myzd<tozd then
	conn.execute "update 用户 set 内力=100,体力=100,武功=100,银两=int(银两/2) where 姓名='" & sjjh_name &"'" &""
	conn.execute "update 用户 set 内力=int(内力/2),银两=银两+"&int(myyin/2)&" where 姓名='"& name &"'"
	dantiao="二人战在一起，战得天昏地暗， {"& sjjh_name &"} 终于不敌，落荒而逃......；["&name&"]得意洋洋得说：这回知道我得厉害了吧！……得到银两："&int(myyin/2)
end if
'我们平手
if myzd=tozd then
	conn.execute "update 用户 set 内力=int(内力/2) where 姓名='"& name &"'"
	conn.execute "update 用户 set 内力=int(内力/2) where 姓名='" & sjjh_name &"'"
	dantiao="二人战在一起，远方天昏地暗， 但是由于二人武功不分上下， 互无胜负，只得约定明天再战，草草收队！……" 
end if
dantiao="【单挑决斗】"&sjjh_name&"马鞭一挥高叫道： "& name &" 你个无名小辈！向我挑战，找死！那我就同你大战三百合，让你尝尝某家得厉害！……<br>"&dantiao
else
	if Application("sjjh_dantiao")<>sjjh_name then
		Response.Write "<script language=JavaScript>{alert('提示：网络超时……');}</script>"
		Response.End 
	end if
dantiao="【免战牌】["&sjjh_name&"]环眼一瞪说：哼！ {"& name &"} 尔等小人，也配同我交手？还是拿镜子自己照照吧！……" 
end if
says=dantiao		'聊天数据
