...
<input type=button name="tbclutch" value="全屏" onClick="javascript:parent.tbclutch();" title="合屏/分屏/垂直切换" style="height: 18;background-color:006699;color:ccffff;border: 1 double" onmouseover="this.style.color='ffffff'" onmouseout="this.style.color='ccffff'">                                                                          
 <input type=button value='僵尸' onClick="window.open('co/cins.asp','f3')" title="释放僵尸王" style="height: 18;background-color:006699;color:ccffff;border: 1 double" onmouseover="this.style.color='ffffff'" onmouseout="this.style.color='ccffff'"><br>                                                                    
   			...
   			<input type="checkbox" name="addvalues" <%if (chatinfo(0)<>"泡点E线" or Instr(LCase(application("sjjh_zanli")),LCase("!"&sjjh_name&"!"))<=0) and sjjh_grade<6 then%>disabled<%end if%>>                                                                          
			<a href="#" title='打开后，系统定时自动发言，自动存点，不在线一样泡，此功能只在泡点房间可用！' onClick='<%if chatinfo(0)<>"泡点E线" and sjjh_grade<6 then%>{alert("此房间禁止使用泡点功能！泡点请到{泡点E线}！")}return false;<%end if%><%if Instr(LCase(application("sjjh_zanli")),LCase("!"&sjjh_name&"!"))<=0 and sjjh_grade<6 then%>{alert("为了保障您的数据安全，请先使用暂离功能！")}return false;<%else%>document.af.addvalues.checked=!(document.af.addvalues.checked);<%end if%>document.af.sytemp.focus();'>泡点</a>                                                                           
