...
if sjjh_jhdj<=12 then
	Response.Write "<script language=JavaScript>{alert('提示：购买需12级以上才可以操作！');}</script>"
	Response.End 
end if
...
if towho<>"大家" and towho<>sjjh_name then
...
rs.close
rs.open "select "&bz&","&lb&" from 用户 where 姓名='"&sjjh_name&"'",conn,3,3
if rs(bz)<yin then
	rs.close
	set rs=nothing
	conn.close
...
conn.execute "update 用户 set "&lb&"='"&zstemp1&"',"&bz&"="&bz&"-"&yin&" where  姓名='"&sjjh_name&"'"
