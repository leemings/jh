...
rs.open "select 法力,等级,道德,grade,体力,智力,职业,操作时间 FROM 用户 WHERE 姓名='" & sjjh_name &"'" ,conn,2,2
...
Response.Write "<script language=JavaScript>{alert('一次最少偷100最多不能超过5000！');}</script>"
rs.close
set rs=nothing	
conn.close
set conn=nothing
...
if rs("职业")<>"魔法师" then
	Response.Write "<script language=JavaScript>{alert('你只是普通子民呢，不能使用偷窃术！！请去职业转换为魔法师！');}</script>"
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
...
if fn1>5000 and rs("grade")<10 then
	Response.Write "<script language=JavaScript>{alert('偷窃术不能大于五千！');}</script>"
	rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
...
if rs("智力")<fn1 then
tqs=to1&"连喊带叫的哭到：" & sjjh_name & "行行好吧，我那有那么多的智力呀，饶了俺吧,大侠!"
else
conn.execute "update 用户 set 法力=法力-500,体力=体力-"& fn1/1 & ",智力=智力+" & fn1 & ",操作时间=now() where 姓名='" & sjjh_name &"'"
conn.execute "update 用户 set 智力=智力-" & fn1 & " where 姓名='"&to1&"'"
tqs="哎…"& to1 & "真可怜,一觉醒变得白痴白痴的原来被人吸走了" & fn1 & "点智力，难怪看起来有点白痴……(" & sjjh_name & "体力下降"& fn1/1 &")"
end if
rs.close
set rs=nothing	
conn.close
