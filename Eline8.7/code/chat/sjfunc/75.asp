...
if mycz="/解除" then
	...
	tongji= "管理员:<font color=red>宣布"&fn1&"已经被官府解除攻击状态，多年的流落生涯今天总算是到头了~~"
else
	if rs("通缉")<>False then
		rs.close
		set rs=nothing
	...
	tongji= "管理员:<font color=red>宣布"&fn1&"事坏作尽，多行不义，现在已被官府通缉，杀死通缉犯的大侠将得到100万~~"
end if
conn.execute "insert into l(b,c,d,a,e) values ('" & sjjh_name & "','" & fn1 & "','操作',now(),'被管理员"&mycz&"')"
rs.close
set rs=nothing	
conn.close
set conn=nothing
