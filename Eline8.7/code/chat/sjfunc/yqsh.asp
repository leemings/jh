...
rs.open "select 操作时间,银两,金币,体力,内力,道德,魅力,性别 FROM 用户 WHERE 姓名='" & sjjh_name &"'",conn,2,2
...
conn.close
set conn=nothing
Response.Write "<script Language=Javascript>alert('请人赏花要300秒一次，请等"& s &"秒,可别累着！');</script>"
response.end
end if
mysex=rs("性别")
rs.close
...
conn.execute "update 用户 set 操作时间=now(),银两=银两-200000,金币=金币-2,体力=体力-8000,内力=内力-5000 where 姓名='"&sjjh_name&"'"
rs.close
set rs=nothing
conn.close
set conn=nothing
...
tw="<bgsound src=wav/ring.WAV loop=1>[##]向{%%}提出了赏花的邀请:<b><a href=../GARDEN/garden.ASP?myid=" & sjjh_name & " target=_blank>来吧，朋友，点击进入我的花园！</a></b>"
end function
%>