...
rs.open "select 内力,名单头像 FROM 用户 WHERE 姓名='" & sjjh_name &"'",conn,2,2
tx=rs("名单头像")
if rs("内力")<1000  then
	rs.close
	set rs=nothing
...
titl="<marquee border='1' onmouseover=this.stop(); onmouseout=this.start();><img src=../ico/"& tx &"-2.gif width=32 height=32>[千里传音]" & fn1 & "  (##)" & "</marquee>"
rs.close
set rs=nothing	
conn.close
set conn=nothing
