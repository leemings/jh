...
rs.open "select 体力,等级,内力加,内力,操作时间 FROM 用户 WHERE 姓名='" & sjjh_name &"'",conn,2,2
...
if rs("内力")<rs("等级")*sjjh_nlsx+2000+rs("内力加") then
	conn.execute "update 用户 set 内力=内力+300,体力=体力-2000,操作时间=now() where 姓名='" & sjjh_name &"'"
...
if rs("内力加")<=rs("等级")*1500 then
	conn.execute "update 用户 set 内力加=内力加+9,体力=体力-2000,操作时间=now() where 姓名='" & sjjh_name &"'"
