...
rs.open "select 内力,体力,等级,体力加,操作时间 FROM 用户 WHERE 姓名='" & sjjh_name &"'",conn,2,2
...
if rs("体力")<rs("等级")*sjjh_tlsx+5260+rs("体力加") then
	conn.execute "update 用户 set 体力=体力+3500,内力=内力-1000,操作时间=now() where 姓名='" & sjjh_name &"'"
...
if rs("体力加")<=rs("等级")*1800 then
	conn.execute "update 用户 set 体力加=体力加+9,内力=内力-1000,操作时间=now() where 姓名='" & sjjh_name &"'"
