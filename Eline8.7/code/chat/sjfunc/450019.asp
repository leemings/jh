...
rs.open "select 门派,w6,w9,法力,等级,操作时间 FROM 用户 WHERE  姓名="&"'" & sjjh_name &"'",conn,3,3
fla=rs("法力")
dj=rs("等级")
w6w=rs("w9")
...
conn.execute "update 用户 set 法力=法力-8000,操作时间=now() where 姓名="&"'" & sjjh_name &"'"
fq=abate(w6w,"绝情刀",1)
...
jiuqingdao="##从腰间拔出一把绝情刀<bgsound src=wav/fx42.wav loop=1>，<img src='img/9.gif' width='44' height='44'>恶狠狠地对着<font color=red>%%</font>刺了下去,受死吧，%%啊的一声，被刺中心脉,曾经沧海难为水啊，体力失去1/3，共计"&money&"点......" 
rs.close
rs.open "select 体力 from 用户 where 姓名="&"'" & towho &"'",conn,2,2
if rs("体力")<=0 then 
	conn.execute "update 用户 set 状态='死' where 姓名="&"'" & towho &"'"
	jiuqingdao="##从腰间拔出一把绝情刀<bgsound src=wav/ZR2199.wav loop=1>，<img src='img/9.gif' width='44' height='44'>恶狠狠地对着<font color=red>%%</font>刺了下去,受死吧,%%啊的一声，由于体力不支，被当场刺死，自古多情空余恨啊..." 
	call boot(towho,"绝情刀，操作者："&sjjh_name&","&fn1)
end if
case else
jiuqingdao="##从腰间拔出一把绝情刀<bgsound src=wav/fx42.wav loop=1>，<img src='img/9.gif' width='44' height='44'>恶狠狠地对着<font color=red>%%</font>刺了下去,受死吧,怎料是爱到深处心恨谁，没刺到啊......"
end select
	rs.close
	set rs=nothing	
	conn.close
