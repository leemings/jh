...
rs.close
rs.open "select 门派,保护,杀人数,grade,w10,死亡时间 from 用户 where 姓名='" & sjjh_name &"'",conn,2,2
if rs("门派")="出家" and sjjh_grade<10 then
	rs.close
	set rs=nothing	
	conn.close
...
if rs("体力")<-100 then
	...
	call boot(sjjh_name,"魔宝，不小心伤了自己！")
end if
rs.close
...
conn.close
set conn=nothing
call boot(to1,"法器，操作者："&sjjh_name&","&zswupin)
end function
%>
