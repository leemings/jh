...
if to1="大家" or to1=Application("sjjh_automanname") then to1=sjjh_name
...
rs.open "select 银两,体力,门派,操作时间 FROM 用户 WHERE 姓名='" & sjjh_name &"'",conn,2,2
...
if rs("银两")<500000 or rs("体力")<1000 or rs("门派")<>"出家" then
	rs.close
	set rs=nothing
	conn.close
	set conn=nothing
...
conn.execute "update 用户 set 银两=银两-500000,体力=体力-1000,操作时间=now() where 姓名='" & sjjh_name &"'"
...
rs.close
rs.open "select 银两,性别,体力,内力,门派,操作时间 FROM 用户 WHERE 姓名='" & sjjh_name &"'",conn,2,2
sj=DateDiff("n",rs("操作时间"),now())
if sj<3 then
	rs.close
	set rs=nothing	
...
if rs("性别")="男" then
	...
	if rs("银两")<100000 or rs("内力")<1000 or rs("门派")<>"出家" then
		rs.close
		set rs=nothing
		conn.close
		set conn=nothing
	...
	conn.execute "update 用户 set 银两=银两-100000,内力=内力-1000,操作时间=now() where 姓名='" & sjjh_name &"'"
