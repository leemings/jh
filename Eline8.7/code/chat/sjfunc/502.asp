...
rs.open "select 内力,职业,体力,等级,道德,武功,武功加,操作时间 FROM 用户 WHERE 姓名='" & sjjh_name &"'" ,conn,2,2
if rs("职业")<>"武师" then
	rs.close
	set rs=nothing	
	conn.close
...
if rs("武功")<rs("等级")*sjjh_wgsx+3800+rs("武功加") then
	conn.execute "update 用户 set 武功=武功+1000,道德=道德-200,操作时间=now() where 姓名='" & sjjh_name &"'"
...
if rs("武功加")<=rs("等级")*1500 then
	conn.execute "update 用户 set 武功加=武功加+50,道德=道德-500,操作时间=now() where 姓名='" & sjjh_name &"'"
	chiwu="武师""##<bgsound src=wav/dz.wav loop=1>为了提升自己的武功，不惜冒着走火入魔的危险进入火云洞<font color=red>终于成功恢复武功</font>50点，道德下降<font color=red>-500</font>点，真是一山要比一山高！"
else
	rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
	Response.Write "<script language=JavaScript>{alert('提示：现在你的上限满了，等升了级再指导武功吧！');}</script>"
	Response.End
end if
rs.close
set rs=nothing	
