...
'if Weekday(date())=7 then
	...
	conn.close
	set conn=nothing
	Response.Write "<script language=JavaScript>{alert('提示：竞标商品只可以在周六20-21点进行！');}</script>"
	Response.End
end if
'对已经竞标的商品结算
rs.open "select r FROM b WHERE a='竞标'",conn,2,2
...
'进行竞标系统
rs.close
rs.open "select a,n,q,o FROM b WHERE n='"&sjjh_name&"'",conn,2,2	
trmoney=0
do while not rs.bof and not rs.eof
...
conn.execute "update b set n='"&sjjh_name&"',q="&money&",o=0 where a='"&wpname&"'"
