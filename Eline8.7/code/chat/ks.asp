...
if sjjh_jhdj>30 then
	Response.Write "<script language=JavaScript>{alert('提示：战斗30级以上者无资格打僵尸王');window.close();}</script>"
	Response.End 
end if
Application.Lock
...
if rs("体力")<-100000 then
	conn.execute "update 用户 set 状态='僵尸王',身份='僵尸王',事件原因='僵尸王|"&fn1&"' where 姓名='" & sjjh_name &"'"
    conn.execute "update 用户 set 体力=5000,内力=5000,事件原因='无',状态='正常',身份='弟子' where 姓名='" & attackname &"'"
  	conn.execute "insert into l(b,a,c,e,d) values ('" & sjjh_name & "',now(),'僵尸王','因勇打僵尸所牺牲','人命')"
	kl="<img src='pic/kl.gif'>["&sjjh_name&"]太衰了，为了跟僵尸王" & attackname & "搏斗结果自己反而被僵尸王"&id&"咬死,["&sjjh_name&"]变成了僵尸王." & attackname & "正式复活各位大虾恭喜他吧" & attackname & "被困在僵尸库好久了..，………"
	call boot(sjjh_name,"僵尸王，操作者：僵尸王，打我呀找死！")
else
dim js(10)
js(0) ="骨头"
js(1) ="蓝宝石"
...
kl="我call,英雄，真是英雄，["&sjjh_name&"]与僵尸王" & attackname & "大打出手……僵尸王终于倒下了，而他自己也伤的不轻，官府奖励<img src='img/251.gif'>"&tempjs*1&"两！在打败僵尸王后，"&sjjh_name&"得到了[<b><font color=red>"&js(myxy)&"</font></b>]2个……"
