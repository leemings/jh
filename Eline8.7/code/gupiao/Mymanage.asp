...
sub Create()
	...
	sqlbbs="select 魅力,银两 from [用户] where 姓名='"&membername&"'"
	set rsbbs=connbbs.execute(sqlbbs)	

'	if rsbbs(0)<clng(KaiHu_Setting(1)) then
'		errmess=errmess+"<li>本股市已限制了开户最少发贴数为"&KaiHu_Setting(1)&"，您现在不符合本要求，暂时不能开户"
'		founderr=true
'	end if
	if rsbbs("魅力")<clng(KaiHu_Setting(2)) then
