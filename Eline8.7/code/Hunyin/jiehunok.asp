...
rs.open "SELECT * FROM 用户 WHERE 姓名='"&sjjh_name&"'"&" and 配偶='无' and 等级>5 and 银两>"&(money+100000),conn
...
sex=trim(rs("性别"))
rs.close
rs.open "SELECT * FROM 用户 WHERE 等级>5 and 姓名='" & name & "' and 配偶='无' and 性别<>'"&sex&"'",conn
if rs.bof or rs.eof then
	rs.close
	set rs=nothing
	conn.close
	...
	conn.execute "update 用户 set 银两=银两-"&(money+100000)&" where 姓名='"&sjjh_name&"'"
	conn.Execute "INSERT INTO h (a,b,c,d,e,f) VALUES('"&sjjh_name&"','"&name&"','结婚',"&money&",now(),'"&mess&"')"
	rs.close
	set rs=nothing
	conn.close
	set conn=nothing
