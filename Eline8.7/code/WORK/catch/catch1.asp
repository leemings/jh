...
select case r
case 1
	mess="恭喜"& sjjh_name &"出海走私捕鱼，捕获了二条大鲨鱼真是可喜可贺<IMG src='../work/catch/fish.gif'>"
	rs.open "SELECT w6 FROM 用户 WHERE 姓名='"&sjjh_name&"'",conn,3,3
	duyao=add(rs("w6"),"大鲨鱼",1)
	conn.execute "update 用户 set  w6='"&duyao&"' where 姓名='"&sjjh_name&"'"
	rs.close
case 2
	mess="恭喜"& sjjh_name &"出海走私捕鱼，意外的拾获到一枚金币真是太棒了<IMG src='img/jinbi.gif'>"
	conn.execute "update 用户 set 银两=银两+50000 where 姓名='"&sjjh_name&"'" 
case 3
	mess="霉运"& sjjh_name &"出海走私捕鱼时遇到一绝色美女，<font color=red><b>风流快活了一夜得了花柳病,魅力增加100,不能在出海走私捕鱼了<IMG src='../work/catch/catch.gif'>"
	rs.open "SELECT 魅力,状态 FROM 用户 WHERE 姓名='"&sjjh_name&"'",conn,3,3
	conn.execute "update 用户 set  魅力=魅力+100,状态='花柳病' where 姓名='"&sjjh_name&"'"
	rs.close
case 4
	mess="霉运"& sjjh_name &"出海走私捕鱼时遇到一绝色美女，<font color=red><b>风流快活了一夜得了梅毒,魅力增加100,不能在出海走私捕鱼了<IMG src='../work/catch/catch.gif'>"
	rs.open "SELECT 魅力,状态 FROM 用户 WHERE 姓名='"&sjjh_name&"'",conn,3,3
	conn.execute "update 用户 set  魅力=魅力+100,状态='梅毒' where 姓名='"&sjjh_name&"'"
	rs.close
case 5
	mess="霉运"& sjjh_name &"出海走私捕鱼时遇到一绝色美女，<font color=red><b>风流快活了一夜得了尿道炎,魅力增加100,不能在出海走私捕鱼了<IMG src='../work/catch/catch.gif'>"
	rs.open "SELECT 魅力,状态 FROM 用户 WHERE 姓名='"&sjjh_name&"'",conn,3,3
	conn.execute "update 用户 set  魅力=魅力+100,状态='尿道炎' where 姓名='"&sjjh_name&"'"
	rs.close
case 6
	mess="恭喜"& sjjh_name &"出海走私捕鱼，捕获了二条大草鱼真是可喜可贺<IMG src='../work/catch/fish.gif'>"
	rs.open "SELECT w6 FROM 用户 WHERE 姓名='"&sjjh_name&"'",conn,3,3
	duyao=add(rs("w6"),"大草鱼",2)
	conn.execute "update 用户 set  w6='"&duyao&"' where 姓名='"&sjjh_name&"'"
	rs.close
case 7
	mess="恭喜"& sjjh_name &"出海走私捕鱼，捕获了二条小鲤鱼真是可喜可贺<IMG src='../work/catch/fish.gif'>"
	rs.open "SELECT w6 FROM 用户 WHERE 姓名='"&sjjh_name&"'",conn,3,3
	duyao=add(rs("w6"),"小鲤鱼",2)
	conn.execute "update 用户 set  w6='"&duyao&"' where 姓名='"&sjjh_name&"'"
case 8	
	mess="霉运"& sjjh_name &"出海走私捕鱼时遇海盗，海盗强走他身上5万银两真是可悲<IMG src='../work/catch/catch.gif'>"
	rs.open "SELECT 魅力,状态 FROM 用户 WHERE 姓名='"&sjjh_name&"'",conn,3,3
	conn.execute "update 用户 set  银两=银两-50000 where 姓名='"&sjjh_name&"'"
	rs.close	
case 9	
	mess="霉运"& sjjh_name &"出海走私捕鱼时遇海盗，海盗把"& sjjh_name &"毒打一顿内力立刻消失5千真是可悲<IMG src='../work/catch/catch.gif'>"
	rs.open "SELECT 内力 FROM 用户 WHERE 姓名='"&sjjh_name&"'",conn,3,3
	conn.execute "update 用户 set  内力=内力-5000 where 姓名='"&sjjh_name&"'"
	rs.close	
case 10	
	mess="霉运"& sjjh_name &"出海走私捕鱼时遇海盗，海盗把"& sjjh_name &"毒打一顿体力立刻消失5千真是可悲<IMG src='../work/catch/catch.gif'>"
	rs.open "SELECT 体力 FROM 用户 WHERE 姓名='"&sjjh_name&"'",conn,3,3
	conn.execute "update 用户 set  体力=体力-5000 where 姓名='"&sjjh_name&"'"
	rs.close
case 11
	mess="恭喜"& sjjh_name &"出海走私捕鱼，捕获了二颗蓝宝石真是可喜可贺<IMG src='img/LOOK52.GIF'>"
	rs.open "SELECT w6 FROM 用户 WHERE 姓名='"&sjjh_name&"'",conn,3,3
	duyao=add(rs("w6"),"蓝宝石",2)
	conn.execute "update 用户 set  w6='"&duyao&"' where 姓名='"&sjjh_name&"'"
case 12
	mess="恭喜"& sjjh_name &"出海走私捕鱼，捕获了二颗绿宝石真是可喜可贺<IMG src='img/LOOK52.GIF'>"
	rs.open "SELECT w6 FROM 用户 WHERE 姓名='"&sjjh_name&"'",conn,3,3
	duyao=add(rs("w6"),"绿宝石",2)
	conn.execute "update 用户 set  w6='"&duyao&"' where 姓名='"&sjjh_name&"'"
case 13
	mess="恭喜"& sjjh_name &"出海走私捕鱼，捕获了二颗红宝石真是可喜可贺<IMG src='img/LOOK52.GIF'>"
	rs.open "SELECT w6 FROM 用户 WHERE 姓名='"&sjjh_name&"'",conn,3,3
	duyao=add(rs("w6"),"红宝石",2)
	conn.execute "update 用户 set  w6='"&duyao&"' where 姓名='"&sjjh_name&"'"
case 14
	mess="恭喜"& sjjh_name &"出海走私捕鱼，捕获了二颗水晶石真是可喜可贺<IMG src='../work/catch/catch.gif'>"
	rs.open "SELECT w6 FROM 用户 WHERE 姓名='"&sjjh_name&"'",conn,3,3
	duyao=add(rs("w6"),"水晶石",2)
	conn.execute "update 用户 set  w6='"&duyao&"' where 姓名='"&sjjh_name&"'"					
		
case 15
	mess="恭喜"& sjjh_name &"出海走私捕鱼！捕获了大鲨鱼、大草鱼、小鲤鱼各一条！<IMG src='../work/catch/fish.gif'>"
	rs.open "SELECT w6 FROM 用户 WHERE 姓名='"&sjjh_name&"'",conn,3,3
	duyao=add(rs("w6"),"大鲨鱼",1)
...
conn.close
set conn=nothing
says="<font color=#ff0000>消息</font>"& sjjh_name &"出海捕鱼："&mess			'聊天数据
says=replace(says,"'","\'")
          ...
          <td height="224" width="163" valign="top">
            ...
            <table border="0" width="165" height="102">
              <tr>
                ...
                <td width="139" height="98" align="center">&nbsp; <img id=face src="../../ico/<%=tx%>-2.gif" alt=个人形象代表 width='70' height='60'></td>  
              </tr>
            </table>
