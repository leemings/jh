...
if request("reaction")<>"" then
	if boardid>0 then
	response.write "当前论坛共有<B>"&allonline()&"</B>位用户在线，其中"&boardtype&"有<B>"&online(boardid,true)&"</B>位在线用户与<B>"&guest(boardid,true)&"</B>位客人"
	else
...
sub onlinemain()
	...
	response.write "<th width=22% >登录时间</th>"
	response.write "</tr>"
	set rs=server.createobject("adodb.recordset")
	...
	if rs.eof and rs.bof then
		...
		response.write "<td align=center class=tablebody1>"&replace(usersysinfo(rs("browser"),2)&","&usersysinfo(rs("browser"),3),"操作系统：","")&"，"&replace(replace(usersysinfo(rs("browser"),1),"浏 览 器：",""),"Internet Explorer","IE")&"</td>"
