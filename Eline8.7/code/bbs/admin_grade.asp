...
比如你新添加了一个用户组，并且给予了这个用户组某一些权限，那么你可以设置达到一定等级（帖子）的用户自动更新到这个用户组以使用这个用户组的权限。<BR>如果您想某个等级的用户不跟随帖子数而上升等级，那么就把最少发帖设置为<B>-1</B>，一般为特殊用户组需要这样的设置，设置某个级别最少发帖为<B>-1</B>后，该级别的用户将不能根据帖子增加而升级，别的用户也不能自动升级到该级别，只有在用户管理中方能更改其级别
</td>
</tr>
</table>
...
<td width="15%" class=forumHeaderBackgroundAlternate><B>最少发帖</B></td>
...
<tr>
<td width="40%" class=forumrow><B>最少发帖</B><BR>如果该等级是荣誉称号或者管理身份，这里可以填写-1，表示不跟随帖子增长而升级</td>
