...
if Cint(GroupSetting(15))=0 then
	Errmsg=Errmsg+"<br>"+"<li>您没有将本页面发送的权限，请<a href=login.asp>登录</a>或者同管理员联系。"
	founderr=true
end if
if request("id")="" then
	founderr=true
...
sub announceinfo()
  ...
  set rs=conndown.execute("select showname from download where ID="&AnnounceID)
	if not(rs.bof and rs.eof) then
		topic="您的朋友"&postname&"给您发来了一个"&Forum_info(0)&"上的软件"
	else
		foundErr = true
	...
	mailbody=mailbody &"您的朋友"&postname&"给您发来了一个"&Forum_info(0)&"上的软件<BR><br>"
	mailbody=mailbody &"标题是："&htmlencode(topic)&"<br><br>"
	mailbody=mailbody &"您可以到<a href=http://"&request.servervariables("server_name")&replace(request.servervariables("script_name"),"z_down_sendpage.asp","")&"list.asp?id="&Announceid&">"&topic&"</a>这里浏览这个帖子<br>"
...
sub pag()%>
	...
	<form action="?action=sendmail&id=<%=announceid%>" method=post>
		<tr>
			<th valign=middle colspan=2>发送邮件给朋友</th>
		</tr>
		<tr>
			<td  class=tablebody1 valign=middle colspan=2><b>通过邮件发送本帖子给您的朋友。</b>　下列所有项必填，并请输入正确的邮件地址！<br>你可以添加一些自己的信息在下面的内容框内。至于这个帖子的主题和 URL 你可以不必写，因为本程序会在发送的 Email 中自动添加的！</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class=tablebody1><textarea name="content" cols="55" rows="6">我想你对 '<%=Forum_info(0)%>' 的这个软件会感兴趣的！请去看看！</textarea></td>
		</tr>
		<tr>
