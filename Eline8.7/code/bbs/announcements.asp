...
%>
<FORM METHOD=POST ACTION="?action=delann&boardid=<%=boardid%>" name="paper">
...
set rs=nothing
%>
<tr><td class=tablebody2 colspan=5 align=right>请选择要删除的公告，<input type=checkbox name=chkall value=on onclick="CheckAll(this.form)">全选 <input type=submit name=Submit value=执行  onclick="{if(confirm('您确定执行此操作吗?')){this.document.paper.submit();return true;}return false;}"></td></tr>
</table>
</FORM>
<%
end sub
...
delid=replace(delid,"--","")
delid=replace(delid,")","")
if not isnull(delid) and delid<>"" then
