...
<%
	...
	case 9
		...
		sql="select top "&request("searchmax")&" u.vip,u.userid,u.username,u.useremail,u.LastLogin,u.UserLastIP,u.article,u.UserGroupID from [user] u inner join UserGroups G on u.usergroupid=g.usergroupid where "&sqlstr&" and u.vip>0 and u.vip<4 order by u.addDate desc"
	case 10
		sql="select u.vip,u.userid,u.username,u.useremail,u.LastLogin,u.UserLastIP,u.article,u.UserGroupID from [user] u inner join UserGroups G on u.usergroupid=g.usergroupid where u.usergroupid="&request("usergroupid")&" and u.vip>0 and u.vip<4 order by u.addDate desc"
	case else
...
<td class=forumHeaderBackgroundAlternate><B>VIP</B></td>
...
<td class=forumrow width=20% align=center><% =vipshow(rs("vip"))%></td>
<td class=forumrow width=18% align=center><a href="?action=UserPermission&userid=<%=rs("userid")%>&username=<%=rs("username")%>">编辑</a></td>
...
<%call DispPageNum(currentpage,PCount,"""?page=","&userSearch="&request("userSearch")&"&username="&request("username")&"&useremail="&request("useremail")&"&homepage="&request("homepage")&"&oicq="&request("oicq")&"&icq="&request("icq")&"&msn="&request("msn")&"&title="&request("title")&"&sign="&request("sign")&"&userclass="&request("userclass")&"&usergroups="&request("usergroups")&"&action="&request("action")&"&usergroupid="&request("usergroupid")&"&searchmax="&request("searchmax")&"""")%>
</td></tr>
<tr><td colspan=5 class=forumrow align=center><B>请选择您需要进行的操作</B>：<br>
    从VIP用户中删除<input type="radio" name="useraction" value=1>&nbsp;&nbsp;删除并限制<input type="radio" name="useraction" value=2>           
    解除限制<input type="radio" name="useraction" value=3>&nbsp; 锁定VIP资格<input type="radio" name="useraction" value=4>&nbsp;               
    解除锁定<input type="radio" name="useraction" value=5 checked>                                  
    <br> 
    &nbsp;                
</td>                        
...
dim sqlvip,sqlb,rsb,boarduser,updateinfo,rsu,sqlu	            
sqlvip="select * from [vip]"            
set rsvip=conn.execute(sqlvip)	            
...
elseif request("action")="touser" then                        
	...
	if not founderr then                        
		if request("useraction")=1 then                        
			conn.execute("update [user] set vip=0 where userid in ("&replace(request("userid"),"'","")&")")  
			response.write "<tr><td colspan=6 class=forumrow>删除操作成功。</td></tr>"    
		elseif request("useraction")=2 then                        
		conn.execute("update [user] set vip=2 where userid in ("&replace(request("userid"),"'","")&")")  
			response.write "<tr><td colspan=6 class=forumrow>删除并限制操作成功。</td></tr>"                        
		elseif  request("useraction")=3 then  
		conn.execute("update [user] set vip=0 where userid in ("&replace(request("userid"),"'","")&")")  
			response.write "<tr><td colspan=6 class=forumrow>解除限制操作成功。</td></tr>"
		elseif request("useraction")=4 then  
		conn.execute("update [user] set vip=3 where userid in ("&replace(request("userid"),"'","")&")")  
			response.write "<tr><td colspan=6 class=forumrow>锁定操作成功。</td></tr>"
		elseif request("useraction")=5 then  
		conn.execute("update [user] set vip=1 where userid in ("&replace(request("userid"),"'","")&")")  
			response.write "<tr><td colspan=6 class=forumrow>解除锁定操作成功。</td></tr>"
		else                       
		response.write "<tr><td colspan=6 class=forumrow>错误的参数。</td></tr>"                        
		end if                        
	end if      
...
<input type=hidden value=0 name="ba2">
