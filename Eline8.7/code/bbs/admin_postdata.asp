...
<table width="95%" border="0" cellspacing="1" cellpadding="3"  align=center class="tableBorder">
<tr> 
<td height="23" colspan="5" class=Forumrow><B>说明</B>：<BR>下列数据表中选中的为当前论坛所使用来保存帖子数据的表，一般情况下每个表中的数据越少论坛帖子显示速度越快，当您下列单个帖子数据表中的数据有超过几万的帖子时不妨新添一个数据表来保存帖子数据（SQL版本用户建议每个表数据达到20万以后进行添加表操作），您会发现论坛速度快很多很多。<BR>您也可以将当前所使用的数据表在下列数据表中切换，当前所使用的帖子数据表即当前论坛用户发帖时默认的保存帖子数据表</td>
</tr>
<tr> 
...
sql="CREATE TABLE "&request.form("tablename")&" (AnnounceID int IDENTITY (1, 1) NOT NULL CONSTRAINT PrimaryKey PRIMARY KEY,"&_
		...
		"PostBuyUser text,"&_
		"speak varchar(50) NULL,"&_
		"score int NULL,"&_
		"scoreuser varchar(50) NULL,"&_
...
conn.execute("create index scoreuser on "&request.form("tablename")&" (scoreuser)")
conn.execute("create index score on "&request.form("tablename")&" (score)")
...
if rs.eof and rs.bof then
	...
	brs.open "select * from "&request("intablename"),conn,1,3
	do while not rs.eof
	'读取导出帖子数据表
	set trs=conn.execute("select * from "&request.form("outtablename")&" where rootid="&rs("topicid")&" order by dateandtime")
	if not (trs.eof and trs.bof) then
	do while not trs.eof
	'插入导入帖子数据表
	brs.addnew
	brs("Boardid")=trs("boardid")
	brs("ParentID")=trs("parentid")
	brs("username")=trs("username")
	brs("topic")=trs("topic")
	brs("body")=trs("body")
	brs("DateAndTime")=trs("dateandtime")
	brs("length")=trs("length")
	brs("rootid")=trs("rootid")
	brs("layer")=trs("layer")
	brs("orders")=trs("orders")
	brs("ip")=trs("ip")
	brs("Expression")=trs("Expression")
	brs("locktopic")=trs("locktopic")
	brs("signflag")=trs("signflag")
	brs("emailflag")=trs("emailflag")
	brs("isbest")=trs("isbest")
	brs("PostUserID")=trs("postuserid")
	brs("speak")=trs("speak")
	brs("score")=trs("score")
	brs("scoreuser")=trs("scoreuser")
	brs("scoretime")=trs("scoretime")
	brs("isagree")=trs("isagree")
	brs("isupload")=trs("isupload")
	brs("isaudit")=trs("isaudit")
	brs("PostBuyUser")=trs("PostBuyUser")
	brs.update
	'更新主题指定帖子表
...
<%
  page_count = page_count + 1
	...
	call DispPageNum(currentpage,PCount,"""?page=","&action=search&keyword="&keyword&"&topic="&request("topic")&"&username="&request("username")&"&tablename="&request("tablename")&"""")
	response.write "</td>"
...
if rs.eof and rs.bof then
	...
	dim brs
	set brs=server.createobject("adodb.recordset")
	brs.open "select * from "&request("totablename"),conn,1,3
	do while not rs.eof
	'取出原表数据
