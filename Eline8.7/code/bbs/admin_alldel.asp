...
<form action="admin_alldel.asp?action=delUser" method="post">
            <tr>
            <td bgcolor="<%=Forum_body(4)%>" valign=middle><font color="<%=Forum_body(7)%>">删除指定日期内没有登录的用户</font></td>
...
<%
	...
	dim movetopic,movepost,movetoday
	movepost=0
	movetoday=0
	for i=0 to ubound(allposttable)
	movepost=movepost+conn.execute("select count(*) from "&allposttable(i)&" where Boardid="&request("outboardid")&" and datediff('d',DateAndTime,Now())>"&request.Form("TimeLimited")&"")(0)
	movetoday=movetoday+conn.execute("select count(*) from "&allposttable(i)&" where Boardid="&request("outboardid")&" and datediff('d',DateAndTime,Now())>"&request.Form("TimeLimited")&" and datediff('d',DateAndTime,Now())=0")(0)
	...
	next
	movetopic=conn.execute("select count(*) from topic where Boardid="&request("outboardid")&" and datediff('d',DateAndTime,Now())>"&request.Form("TimeLimited")&"")(0)
	...
	conn.execute("update board set lastbbsnum=lastbbsnum-"&movepost&",lasttopicnum=lasttopicnum-"&movetopic&",todaynum=todaynum-"&movetoday&" where Boardid="&request("outboardid")&"")
	conn.execute("update board set lastbbsnum=lastbbsnum+"&movepost&",lasttopicnum=lasttopicnum+"&movetopic&",todaynum=todaynum+"&movetoday&" where Boardid="&request("inboardid")&"")
	response.write "移动成功！"
	end sub
%>