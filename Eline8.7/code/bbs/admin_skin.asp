...
<%
	set rs=conn.execute("select id,active,skinname from config order by id")
	do while not rs.eof
%>
<tr> 
...
%>
                <tr> 
                  <td width="80%" height=22 align=right class=forumrow><font color="<%=Forum_body(8)%>">&nbsp;当前论坛默认使用模板只能选择一个&nbsp;</font></td><td width="50%" class=forumrow><input type=submit value="提 交" name=submit></td>
                </tr>
	       </table>
		   </form>
<%
