...
sub announceinfo()
	...
	if not(rs.bof and rs.eof) then
		topic="您的朋友"&postname&"给您发来了一个"&Forum_info(0)&"上的帖子"
	else
		foundErr = true
	...
	mailbody=mailbody &"您的朋友"&postname&"给您发来了一个"&Forum_info(0)&"--"&boardtype&"上的帖子<BR><br>"
	mailbody=mailbody &"标题是："&htmlencode(topic)&"<br><br>"
	mailbody=mailbody &""&htmlencode(content)&"<br><br>"
	mailbody=mailbody &"您可以到<a href=http://"&request.servervariables("server_name")&replace(request.servervariables("script_name"),"sendpage.asp","")&"dispbbs.asp?boardid="&boardid&"&id="&Announceid&">"&topic&"</a>这里浏览这个帖子<br>"
