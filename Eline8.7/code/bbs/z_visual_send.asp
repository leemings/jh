	...
	if founderr then
		...
		if request("action")="send" then
			if request("myself")=1 then
				...
				rs("content")=MsgBody&chr(10)&chr(10)&"商品已经放到您的储物柜中，请笑纳！"&chr(10)&"请到以下地址查看储物柜。"&chr(10)&"[align=center][URL=z_visual.asp?shopid=100][color=blue][b][u]查看储物柜[/u][/b][/color][/URL][/align]"
				rs("flag")=0 
				rs("issend")=1
				rs.update
				rs.close
			...
			<table cellpadding=3 cellspacing=1 align=center bgcolor=<%=Forum_Body(27)%> width=97%>
         	...
         	<td valign=middle class=tablebody1 baseName=images/img_visual/blank.gif width="84" height="84" border="0" itemgender="" itemno=<%=CurItem%> layerno="" localpic=<%=LocalPic%> ImagePath="<%=PicPath%>" style="behavior:url(inc/z_show2.htc)"></td>
				...
				<tr> 
					<td class=tablebody1 colspan=3><b>说明</b>：<br>① 您可以使用<b>Ctrl+Enter</b>键快捷发送赠言<br>② 内容最多<b><%=GroupSetting(34)%></b>个字符<br><font color=<%=forum_body(8)%>>③ 赠送该商品的手续费为：<%=SendFee%> 元<%
						if request("fromuser")<>"" then
...
sub send_head()%>
	...
	<title><%=Forum_info(0)%>--赠送形象商品</title>
