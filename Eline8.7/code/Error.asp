...
Case "000"%>
      ...
      <p>　　对不起，本程序不能在这台服务器上运行，如果你是本程序的合法用户，请立即与作者联系。<br>
        信箱：<a href="mailto:yxt_email@etang.com">yxt_email@etang.com</a><br>
      </p>
      <%Case "003"%>
      ...
      <%Case "101"%>
      <p>　　欢迎您的光临，聊天室现在已满，站长限制最多同时在线人数为 <font color=red><%=Application("sjjh_chat_maxpeople")%></font> 
        人，请稍后再来。</p>
      <%Case "102"%>
      <p>　　站长禁止新用户名登录，请稍后再来。</p>
      <%Case "110"%>
      <p>　　您现在的 IP：<font color=red><%=Request.ServerVariables("REMOTE_ADDR")%></font> 
        被封锁 <%=Application("sjjh_iplocktime")%> 分钟，不能进入聊天室。<br>
        　　离自动解锁时间还有：<font color=red><%=ABS(int(Application("sjjh_iplocktime"))-int(Datediff("s",Request.QueryString("lockdate"),now())/60))%></font> 
        分钟。</p>
      <%Case "111"%>
      <p>　　您现在的 IP：<font color=red><%=Request.ServerVariables("REMOTE_ADDR")%></font> 
      ...
      <p>　　密码不能和用户名相同。</p>
      <%Case "130"%>
      <p>　　对不起，该用户名为系统所保留，您不能使用这个名字登录。</p>
      <%Case "131"%>
      <p>　　对不起，该用户名含有不雅字眼，您不能使用这个名字登录。</p>
      <%Case "132"%>
      <p>　　对不起，称谓含有不雅字眼，您不能使用这个称谓登录。</p>
      <%Case "140"%>
      ...
      <p>　　该程序执行了非法操作，请立即停止使用，否则可能引起不可预测的后果，请立即与作者取得联系。<br>
        信箱：<a href="mailto:yxt_email@etang.com?subject=%D3%D0%B9%D8E%CF%DF%BD%AD%BA%FE%B5%C4%CE%CA%CC%E2">yxt_email@etang.com</a><br>
      </p>
      <%Case "200"%>
      ...
      <%Case "440"%>
      <p>　　对不起！您还没有<a href=go.asp target=_blank>登录</a></p>
      <%Case "441"%>
      <p>　　未找到掌门的资料，掌门未定！</p>
      <%Case "442"%>
      ...
      <p>　　错误：操作不成功!!请返回！</p>
      <%Case "460"%>
      <p>　　错误：你条件还没到呢，怎么就想创派了？！回头认真看看具体条件吧！</p>
      <%Case "461"%>
      ...
      <%Case "485"%>
      <p>　　加入官府一定要战斗等级超过65级,魅力和道德超过10万,银两10亿！</p>
      <%Case "490"%>
      <p>　　你不累吗？先坐下来休息一下养足精神，再继续吧！</p>
      <%Case "491"%>
      <p>　　你的金属性为空，请到江湖论坛专帖登记，等站长解决！</p>
      <%Case "492"%>
      <p>一天最多发两次，别发太多啦！</p>
      <%Case else%>
