...
njd=rs("j")
rs.close
rs.open "select 会员等级,银两,操作时间,金币 from 用户 where 姓名='"&sjjh_name&"' and "&tj,conn,3,3
...
conn.execute "update 用户 set 银两=银两-" & yin & ",金币=金币-"&jinbi&",操作时间=now() where 姓名='"&sjjh_name&"'"
conn.execute "insert into vh(a,b,c,d,e,f,g,h,i,l) values('"&wpname&"','"&sjjh_name&"',false,'无',false,'无','"&wpname&"','"&wpwj&"',"&njd&",date())"
rs.close
set rs=nothing
conn.close
set conn=nothing
if hy>0 then
	Response.Write "<script Language=Javascript>alert('提示：会员购买物品银两打5折！购买"&wpname&"成功！你现在可以去定制进入聊天室的公告了！');location.href = 'javascript:history.go(-1)';</script>"
else
	Response.Write "<script Language=Javascript>alert('提示：购买"&wpname&"成功,如果您是会员银两可以打5折！你现在可以去定制进入聊天室的公告了！');location.href = 'javascript:history.go(-1)';</script>"
end if
%>
