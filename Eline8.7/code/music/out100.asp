    ...
    <tr> 
      ...
      <td width="750">
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
          <tr bgcolor="#F7FBFF"> 
            <td width="75%" height="30"><b><font color="#009900">劲爆歌曲100首</font></b></td>
...
set rs=conn.execute("SELECT top 100 id,Wma,Singer,ClassID,SClassID,NClassID,MusicName,hits FROM MusicList order by hits asc") 
...
%>
                    <tr bgcolor="#F7FBFF"> 
                      ...
                      <td> 　<%=i%>.<a href="#" onClick="window.open('YxPlay.asp?id=<%=rs("id")%>','Ting88','scrollbars=no,resizable=no,width=300,height=300,menubar=no,top=168,left=168')" ><%=rs("MusicName")%></a></td>
                      <td align=center><%=rs("hits")%></td>
                      <td align=center> 
                        <%if rs("Wma")<>"" then%>
                        <a href="#" onClick="window.open('YxPlay.asp?id=<%=rs("id")%>','Ting88','scrollbars=no,resizable=no,width=300,height=300,menubar=no,top=168,left=168')" >试听</a> 
                        <%else%>
                        暂无 
                        <%end if%>
                      </td>
