...
<form name="form1" method="POST" action="SongConSave2.asp"> 
...
str+='歌曲名称'+i+':<input name="MusicName'+i+'" size=20><input type="text" name="Rm'+i+'" value="'+i+'.Rm" size=6><select name="Classic'+i+'" size="1"><option name=option<%=rs("Classid")%><%=i%> selected value="<%=rs("Classid")%>,<%=rs("SClassid")%>,<%=rs("NClassid")%>,<%=rs("Specialid")%>"><%=rs("Name")%></option></select><br><br>'; 
