<html> 
<body topmargin="0" leftmargin="0" oncontextmenu=window.event.returnValue=false onselectstart=event.returnValue=false ondragstart=window.event.returnValue=false>

<script language="JavaScript"> 
<!-- 
function defaul_home(){this.home.style.behavior='url(#default#homepage)';this.home.setHomePage('http://www.06net.com');}//--> 
</script>
<div align="center">
  <center>
  <td width="766" height="21"><TABLE cellSpacing=0 cellPadding=0 width=777 border=0>
        <TBODY>
          <TR> 
            <TD><IMG height=1 
      src="images/spacer.gif" width=395 
      border=0 name=undefined_3></TD>
            <TD><IMG height=1 
      src="images/spacer.gif" width=382 
      border=0 name=undefined_3></TD>
            <TD><IMG height=1 
      src="images/spacer.gif" width=1 
      border=0 name=undefined_3></TD>
          </TR>
          <TR> 
            <TD height="9"><IMG height=8 
      src="images/copy_r1_c1.gif" width=395 
      border=0 name=copy_r1_c1></TD>
            <TD><IMG height=8 
      src="images/copy_r1_c2.gif" width=382 
      border=0 name=copy_r1_c2></TD>
            <TD><IMG height=8 
      src="images/spacer.gif" width=1 
      border=0 name=undefined_3></TD>
          </TR>
          <TR> 
            <TD background="images/copy_r2_c1.gif" 
    colSpan=2> <DIV align=center>  
                CopyRight &copy; 2003-2004 <a href=http://www.51eline.com><font face=Verdana, Arial, Helvetica, sans-serif size=1><b>wWw.<font color=#CC0000>51Eline</font>.CoM</b></font></a></DIV></TD>
            <TD><IMG height=18 
      src="images/spacer.gif" width=1 
      border=0 name=undefined_3></TD>
          </TR>
          <TR> 
            <TD colSpan=2><IMG height=5 
      src="images/copy_r3_c1.gif" width=777 
      border=0 name=copy_r3_c1></TD>
            <TD><IMG height=5 
      src="images/spacer.gif" width=1 
      border=0 name=undefined_3></TD>
          </TR>
          <TR> 
            <TD background="images/copy_r4_c1.gif" 
    colSpan=2> <div align="center"><font color="#FFFFFF">|</font> <a href="www.51eline.com" target="_blank"><font color="#FFFFFF">关于我们</font></a><font color="#FFFFFF"> 
                | </font><a href="../bbs/messanger.asp?action=new&touser=一线天" target="_blank"><font color="#FFFFFF">给我留言</font></a> 
                <font color="#FFFFFF">|</font> <a href="mailto:Eline_Email@etang.com"><font color="#FFFFFF">联系我们</font></a><font color="#FFFFFF"> 
                |</font></font> 
              </div><TD><IMG height=19 
      src="images/spacer.gif" width=1 
      border=0 name=undefined_3></TD>
          </TR>
          <TR> 
            <TD colSpan=2><img height=14 
      src="images/copy_r5_c1.gif" width=777 
      border=0 name=copy_r5_c1></TD>
            <TD><IMG height=14 
      src="images/spacer.gif" width=1 
      border=0 name=undefined_3></TD>
          </TR>
        </TBODY>
      </TABLE></td>
    </center>
</div>
</body>                                                                                                 
<script src=images/Js.js></script>
</html>