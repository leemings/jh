...
if rq<>29 and rq<>30 and rq<>31 then

	Response.Write "<script Language=Javascript>alert('月末礼物只能29,30,31日3天来领取,今天是"&rq&"号！');location.href = 'javascript:history.go(-1)';</script>"
	response.end
end if
...
if rs("mvalue")<=70000 then
	rs.close
	set rs=nothing
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('月积分在70000以上的才可以领取物品！');location.href = 'javascript:history.go(-1)';</script>"
	response.end
end if
myid=rs("id")
wpdate=rs("w5")
...
rs1.open "select * from 月末礼物 where yhid="&myid,conn1
...
select case r
	...
	case 13 '攻防卡
		n=int(rnd*6)+7
		temp=add(wpdate,"攻防卡",n)
		sql="update 用户 set w5='"&temp&"' where id="&myid
		lw="攻防卡"&n&"个"
end select
set rs=conn.execute(sql)
conn1.execute "insert into  月末礼物(yhid,姓名,礼物) values ('"&myid&"','"&aqjh_name&"','"&lw&"')"
set rs=nothing
conn.close
set conn=nothing
set rs1=nothing
conn1.close
set conn1=nothing
says="<font color=red><b>【月末礼物】</b></font>"&aqjh_name&"从站长那里得到月末礼物"&lw&""
