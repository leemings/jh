...
if weekdate<>1 then
	Response.Write "<script Language=Javascript>alert('周日礼物只能周日领取今天是礼拜"&weekdate-1&"！');location.href = 'javascript:history.go(-1)';</script>"
	response.end
end if
...
if rs("mvalue")<=40000 then
	...
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('月积分在40000以上的才可以领取物品！');location.href = 'javascript:history.go(-1)';</script>"
	response.end
end if
myid=rs("id")
wpdate=rs("w5")
...
rs1.open "select * from 周日礼物 where yhid="&myid,conn1
...
conn1.execute "insert into  周日礼物(yhid,姓名,礼物) values ('"&myid&"','"&aqjh_name&"','"&lw&"')"
set rs=nothing
conn.close
set conn=nothing
set rs1=nothing
conn1.close
set conn1=nothing
says="<font color=red><b>【周日礼物】</b></font>"&aqjh_name&"从站长那里得到周日礼物"&lw&""
