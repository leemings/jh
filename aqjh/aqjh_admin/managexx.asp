...
<form method="post" action="managexxok.asp" name="setmp" onsubmit="return(check());">
  <table border="0" cellspacing="1" cellpadding="4" bgcolor="f2f2ea" align="center" width="70%">
    <tr> 
      <td width="67"> 
      ...
      <td width="197"> <%=rs("id")%>  
        ...
        等级:<%=(int(sqr(rs("e")/100))+1)%> </td>
      <td width="81"> 
        <div align="center">轩辕名</div>
      </td>
      <td width="183"> 
...
value="<%=rs("d")%>" size="15" maxlength="3" class="form" style="BORDER-BOTTOM:#B8AF86 1px solid">
    ...
    <tr> 
      <td width="67"> 
        <div align="center">修改等级</div>
      </td>
      <td width="197"> 
        <input type="text" name="e"
value="<%=rs("e")%>" size="15" maxlength="7" class="form" style="BORDER-BOTTOM:#B8AF86 1px solid">
	</td>
    </tr>
    <tr> 
      <td width="67" nowrap> 
...
招式图片：       
        <select name=g size=1 onChange="document.images['face'].src='../chat/xx/gif/'+options[selectedIndex].value+'';" style="BACKGROUND-COLOR: #99CCFF; BORDER-BOTTOM: 1px double; BORDER-LEFT: 1px double; BORDER-RIGHT: 1px double; BORDER-TOP: 1px double; COLOR: #000000">      
   <option value="随机" selected>随机</option>
          <%for t=1 to 24%>
          <option value="wg<%=t%>.gif">wg<%=t%>.gif</option>
			<%next%>
	    </select>      
        <img id=face src="../chat/xx/gif/wg1.gif" alt=招式图片> </td>      
    </tr>      
    <tr>       
