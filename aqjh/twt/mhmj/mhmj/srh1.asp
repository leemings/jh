...
if InStr(http,"mhmj/srh.asp")=0 then 
...
if nl<160000 then 
	Response.Write "<script Language=Javascript>alert('提示：内力小于160000不能进入！');location.href = 'javascript:history.go(-1)';</script>"
	Response.End
end if
if fl<50000 then 
	Response.Write "<script Language=Javascript>alert('提示：法力小于50000不能进入！');location.href = 'javascript:history.go(-1)';</script>"
	Response.End
end if
dim js(21)
js(0) ="骨头"
...
if gwtl<0 then
	Response.Write "<script Language=Javascript>alert('提示："&aqjh_name&"恭喜你！"&gwm&"已经被你成功消灭，获得"&js(myxy)&"5个，"&js1(myxx)&"25个，金币20个。积分（存点）"&s3&"点！！');location.href = 'javascript:history.go(-1)';</script>"
else
