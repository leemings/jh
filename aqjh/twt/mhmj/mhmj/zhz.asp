...
if InStr(http,"mhmj/xzw.asp")=0 then 
...
if fl<80000 then 
	Response.Write "<script Language=Javascript>alert('提示：召唤怪物需要80000点法力！');location.href = 'javascript:history.go(-1)';</script>"
	Response.End
end if
rs.close
...
conn.execute "update 怪物 set 体力=13000000,杀气=4000000,攻击=3000000,防御=2000000,等级=100,状态='正常'  where 姓名='"&gwm&"'"
conn.execute "update 用户 set 法力=法力-80000 where 姓名='"&aqjh_name&"'"
rs.close
set rs=nothing
conn.close
set conn=nothing
says="<font color=RED><b>〖唤醒怪物〗</b></font><font color=green>[<font color=red><b>"& aqjh_name &"</b></font>]耗费[<font color=red><b>80000</b></font>]点法力成功的将已经死亡的[<font color=red><b>"&gwm&"</b></font>]唤醒！看来江湖又将是一场血雨腥风！不知道又将有多少江湖义士埋骨荒野。。。</font>"			'聊天数据
says=replace(says,"'","\'")
...
End Sub
%>
<script Language=Javascript>alert('提示：<%=aqjh_name%>口中念念有词，耗费80000点法力将<%=gwm%>成功唤醒！');location.href = 'javascript:history.go(-1)';</script>
