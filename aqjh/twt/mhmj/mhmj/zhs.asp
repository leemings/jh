...
if fl<50000 then 
	Response.Write "<script Language=Javascript>alert('提示：召唤怪物需要50000点法力！');location.href = 'javascript:history.go(-1)';</script>"
	Response.End
end if
rs.close
...
conn.execute "update 怪物 set 体力=5000000,杀气=2000000,攻击=1000000,防御=800000,等级=50,状态='正常'  where 姓名='"&gwm&"'"
conn.execute "update 用户 set 法力=法力-50000 where 姓名='"&aqjh_name&"'"
rs.close
set rs=nothing
conn.close
set conn=nothing
says="<font color=RED><b>〖唤醒怪物〗</b></font><font color=green>[<font color=red><b>"& aqjh_name &"</b></font>]耗费[<font color=red><b>50000</b></font>]点法力成功的将已经死亡的[<font color=red><b>"&gwm&"</b></font>]唤醒！看来江湖又将是一场血雨腥风！不知道又将有多少江湖义士埋骨荒野。。。</font>"			'聊天数据
says=replace(says,"'","\'")
...
End Sub
%>
<script Language=Javascript>alert('提示：<%=aqjh_name%>口中念念有词，耗费50000点法力将<%=gwm%>成功唤醒！');location.href = 'javascript:history.go(-1)';</script>
