...
if nl<18000 then 
	Response.Write "<script Language=Javascript>alert('提示：内力小于18000不能进入！');location.href = 'javascript:history.go(-1)';</script>"
	Response.End
end if
if fl<60000 then 
	Response.Write "<script Language=Javascript>alert('提示：法力小于60000不能进入！');location.href = 'javascript:history.go(-1)';</script>"
	Response.End
end if
dim js(21)
js(0) ="骨头"
...
conn.execute "update 用户 set 金币=金币+25,allvalue=allvalue+"&s3&",w1='"&zstemp1&"',w6='"&zstemp&"',操作时间=now() where 姓名='"&aqjh_name&"'"
end if
if gwtl<0 then
	Response.Write "<script Language=Javascript>alert('提示："&aqjh_name&"恭喜你！"&gwm&"已经被你成功消灭，获得"&js(myxy)&"7个，"&js1(myxx)&"30个，金币25个。积分（存点）"&s3&"点！！');location.href = 'javascript:history.go(-1)';</script>"
else
