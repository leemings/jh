...
if rs("体力")<20 then	
	...
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('你疲劳程度已超出范围，为防不测，还是离开此地为上！');location.href = 'javascript:history.go(-1)';</script>"
	Response.End
end if
...
says="<font color=#ff0000>【风雪舞剑】</font>"&message
says=replace(says,"'","\'")
...
Application.UnLock()
End Sub
Response.Write "<script language=JavaScript>{alert('提示："&message&"');location.href='FENGXUE.ASP'}</script>"
	Response.End 
%>