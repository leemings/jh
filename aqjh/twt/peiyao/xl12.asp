...
rs.open "SELECT id FROM w WHERE b="& aqjh_id & " and i>=15 and a='矿石'",conn
...
iceid=rs("id")
rs.close
rs.open "SELECT id FROM w WHERE b=" & aqjh_id & " and i>=5 and a='大鲨鱼'",conn
if rs.eof or rs.bof then 
	rs.close
	set rs=nothing
	conn.close
...
rs.close
rs.open "select id from x where a='舍利子' and b="&aqjh_id
If Rs.Bof OR Rs.Eof then
	conn.execute "insert into x(a,b,c,d,e) values ('舍利子'," & aqjh_id &",1,'杀人数',0)"
else
	id=rs("id")
