...
Response.Write "<script language=JavaScript>{alert('错误，提交数据非法！');window.close();}</script>"
Response.End 
end if
if nkeycode="" or nkeycode=null or nkeycode="关键字" then
Response.Redirect "list.asp"
end if
sql="select * from 公告 where "
tj=""
if btnr=0 and zblb=0 then
tj=tj&" instr(标题,'"&nkeycode&"')<>0 or instr(内容,'"&nkeycode&"')<>0"
else
if btnr=1 then
tj=tj&" instr(标题,'"&nkeycode&"')<>0"
elseif btnr=2 then
tj=tj&" instr(内容,'"&nkeycode&"')<>0"
end if
if zdlb<>0 then
if btnr=0 then
tj=tj&" 所属类别="&zdlb
    ...
    <TD background=../index1.files/tstdn_9.jpg>
      ...
      border=0>
        ...
        <TR>
          <TD height=27>
            <DIV align=right>
            <P align=center><A href="http://www.7758530.com">首页</A> <A 
            href="http://www.7758530.com/bbs/" target=_blank>江湖论坛</A> <A 
...
<br>
<select name="btnr">
<option value="0" <%if btnr=0 then%>selected<%end if%>>不指定条件</option>
<option value="1" <%if btnr=1 then%>selected<%end if%>>按标题搜索</option>
<option value="2" <%if btnr=2 then%>selected<%end if%>>按内容搜索</option>
</select>
<br>
<%rs.open "select * from l"%>
<br>
<select name="zdlb">
<option value="0" <%if zdlb=0 then%>selected<%end if%>>不指定类别</option>
...
&nbsp;&nbsp;◆&nbsp;<a href="#" onclick="window.open('disp.asp?id=<%=rs("id")%>','','scrollbars=yes,resizable=yes,width=670,height=500,menubar=no')" title=<%=rs("标题")%>><%=rmbt%></a><br>
<%rs.movenext
loop
rs.close%>
</td>
...
<TBODY> 
<%dim tmprs
sql1="select count(*) as 标题 from 公告 where "&tj
...
count=0%>
<TR> 
<TD colspan="4" vAlign=top height=62>按指定关键字：<font color=red><%=nkeycode%></font>，共搜索到<font color=red><%=musers%></font>条记录<br>
<br>
...
<div align="center">共 <%=musers%> 条，当前第 <%=page%>/<%=pgnum%> 页，每页 
<%=pagesize%> 条<br>
<%if page>1 then%>
<a href="search.asp?btnr=<%=btnr%>&zdlb=<%=zdlb%>&nkeycode=<%=nkeycode%>&page=<%=page-1%>">上一页</a> 
<%end if
for i=1 to pgnum
if page=i then%>
<b>[<%=i%>]</b> 
...
next
if page<pgnum then%>
<a href="search.asp?btnr=<%=btnr%>&zdlb=<%=zdlb%>&nkeycode=<%=nkeycode%>&page=<%=page+1%>">下一页</a> 
<%end if%>
</div>
</TD>
</TR>
