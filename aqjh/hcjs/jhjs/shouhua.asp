...
<html>
<head>
<title><%=Application("aqjh_chatroomname")%>鲜花收购处</title>
...
%> 
  <tr bgcolor="#dfefff" onmouseout="this.bgColor='#DFEFFF';"onmouseover="this.bgColor='#85C2E0';">  
    ...
    <td> 
      <div align="right"><%=rs("h")%><%if data3(0)="冰雪" or data3(0)="逸韵" or data3(0)="香芸" or data3(0)="忧梦" or data3(0)="烈焰" then%>个<%else%>两<%end if%>/个</div> 
    </td> 
    <td> 
      <div align="right"><%if data3(0)="冰雪" or data3(0)="逸韵" or data3(0)="香芸" or data3(0)="忧梦" or data3(0)="烈焰" then%><%=int(rs("h")/4)%>个<%else%><%=int(rs("h")/3)%>两<%end if%>/个 
      </div> 
    </td> 
    <td>  
