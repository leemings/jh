...
<table width="84%" border="0" height="205">
  <tr> 
    ...
    <td height="31" colspan="2"> 
      <div align="center"><img src="huaimg/petbg1.gif" width="188" height="35"></div>
    </td>
...
rs.open "SELECT * FROM b where b='小孩' order by b,h",conn,3,3
Response.Write "<tr>"
...
%>      </table>
  ...
  <tr> 
    ...
    <td colspan="3" height="64" id="zl">嗨，快把自己喜欢的小孩领回家吧。</td>
  </tr>
</table>
<script language=javascript>
function showwp(name,money,jb,sm){
	zl.innerHTML='<FORM action=buyhw.asp method=post name=form1><input type=hidden name=huaname value='+name+'>[<font color=blue>'+name+'</font>]，('+sm+')价值为：'+money+'两,金币：'+jb+'个! <img src="huaimg/yes0.gif" onclick=yesno("'+name+'") ></form> '
}
function yesno(name) {
...
function count() {
	...
	if (random>0&&random<0.1&&dialogcount>0)
		zl.innerHTML="<font color=blue>买了小孩可要好好养他们哟！</font>";
	if (random>=0.1&&random<0.2&&dialogcount>0)
		zl.innerHTML="<font color=blue>每一个小孩都有他们的特性。</font>";
	if (random>=0.2&&random<0.3&&dialogcount>0)
		zl.innerHTML="<font color=blue>小孩可以攻击的，攻击力在于你的培养！</font>";
	if (random>=0.3&&random<0.4&&dialogcount>0)
		zl.innerHTML="<font color=blue>如果你是会员，可以选择会员小孩。</font>";
	if (random>=0.4&&random<0.5&&dialogcount>0)
		zl.innerHTML="<font color=blue>对小孩你要倍加爱护，否则他会死的。</font>";
	if (random>=0.5&&random<0.6&&dialogcount>0)
		zl.innerHTML="<font color=blue>小孩是每5小时照顾一次。</font>";
	...
	if (random>=0.7&&random<0.8&&dialogcount>0)
		zl.innerHTML="<font color=blue>当小孩心情不好时他可不听话的。</font>";
	if (random>=0.8&&random<0.9&&dialogcount>0)
		zl.innerHTML="<font color=blue>买小孩吧，看他多可爱。</font>";
