...
huaid=rs("id")
rs.close
rs.open "select 会员等级,银两,操作时间,w7,金币 from 用户 where 姓名='"&aqjh_name&"' and "&tj,conn,3,3
...
conn.execute "update 用户 set 银两=银两-" & yin & ",金币=金币-"&jinbi&",操作时间=now(),w7='"&zstemp&"' where 姓名='"&aqjh_name&"'"
...
<%
if hy>0 then
	Response.Write "<script Language=Javascript>alert('提示：你是"&hy&"级会员，购买物品打"&hygm*10&"折,购买"&huaname&huasl&"个成功！');location.href = 'javascript:history.go(-1)';</script>"
else
	Response.Write "<script Language=Javascript>alert('提示：购买"&huaname&huasl&"个成功,如果您是会员可以打折！');location.href = 'javascript:history.go(-1)';</script>"
end if
%>
