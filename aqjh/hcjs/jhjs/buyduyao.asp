...
jinbi=rs("m")
rs.close
rs.open "select 会员等级,银两,操作时间,w2,金币 from 用户 where 姓名='"&aqjh_name&"' and "&tj,conn,3,3
...
conn.execute "update 用户 set 银两=银两-" & yin & ",金币=金币-"&jinbi&",操作时间=now(),w2='"&zstemp&"' where 姓名='"&aqjh_name&"'"
rs.close
set rs=nothing
conn.close
set conn=nothing
