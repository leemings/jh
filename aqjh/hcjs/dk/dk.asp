...
conn.execute "update 用户 set 银两=银两+"&dk&" where 姓名='"&aqjh_name&"'"
rs.close
set rs=nothing
conn.Close
set conn=nothing
...
'贷款操作项在此结束
	...
	dk=rs("c")
	rs.close
	rs.open "select * from 用户 where 姓名='"&aqjh_name&"'"
	qian=rs("银两")
	q=qian-(dk*1.5)
if q<0 then
	response.redirect "daikuan.asp"
...
conn.execute "update 用户 set 银两=银两-"&dk*1.5&" where 姓名='"&aqjh_name&"'"
rs.close
set rs=nothing
conn.close
set conn=nothing
