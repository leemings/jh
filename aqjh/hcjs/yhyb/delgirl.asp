...
if rs.eof or rs.bof then
	...
	connt.close
	set connt=nothing
	Response.Write "<script Language=Javascript>alert('你弄错了吧，本院没有你这样的人呀！');window.close();</script>"
	response.end
end if
if yinliang<100000000 then
	...
	connt.close
	set connt=nothing
	Response.Write "<script Language=Javascript>alert('你的腰包里没有足够的钱，想来赎身，门都没！');window.close();</script>"
	response.end
end if
conn.execute "update 用户 set 银两=银两-100000000 where 姓名='"& aqjh_name &"'"
connt.execute("delete * from 妓男 where 妓男ID="&aqjh_id)
rs.close
set rs=nothing
conn.close
set conn=nothing
