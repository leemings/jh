...
if rs.eof or rs.bof then
	...
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('你不是江湖中人，不可进入人妖馆！');window.close();</script>"
	response.end
end if
yl=rs("银两") 
sex=rs("性别")
...
if rs("内力")>=rs("等级")*aqjh_nlsx+2000+rs("内力加") then 
	...
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('对不起，你的内力已达到你的上限，不能再进馆了！');window.close();</script>"
	response.end
end if
if sex="男" then
	rs.close
	...
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('你是男的也来这种地方,小心抓你去做妓男！别跑!');window.close();</script>"
	response.end
end if
...
if sj<120 then
	...
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('你是不是刚从人妖馆里出来呀？请等上"&ss&"秒再来吧！');location.href = 'javascript:history.go(-1)';</script>"
	Response.End
end if
%>
...
if rs.eof or rs.bof then
	...
	connt.close
	set connt=nothing
	Response.Write "<script Language=Javascript>alert('你有没有搞错呀，本馆内哪有这个人呀，是不是想他想疯了，那就卖他进来呗!');window.close();</script>"
	response.end
end if
jiid=rs("妓男ID")
mingji=rs("姓名")
...
select case r
	case 1
		mess=aqjh_name &"在人妖馆观看"&mingji&"歌舞表演，"&aqjh_name&"体力消耗五百点，魅力增加100点，内力增加"& nlj &"点，付给"&mingji&"银两"&int(yin)&"，人妖馆从中收取一半银两，"&mingji&"美貌度增加10点！"
		conn.execute "update 用户 set 银两=银两-" & yin & ",魅力=魅力+100,体力=体力-500,内力=" & newnl & ",操作时间=now() where 姓名='" & aqjh_name & "'"
		
		connt.execute "update 妓男 set 美貌度=美貌度+10 where id=" & id
	case 2
		mess=aqjh_name &"在人妖馆和"&mingji&"聊天，话不投机，气愤之余居然把"&mingji&"捆得粽子一样，吊在梁上胖揍一顿，"&mingji&"的惨叫声十里可闻。惨啊。"& aqjh_name &"道德下降100点，体力下降300点，还白白花了"& yin &"两银子。"& mingji &"美貌度下降10点！"
		conn.execute "update 用户 set 银两=银两-" & yin & ",道德=道德-100,体力=体力-300,操作时间=now() where 姓名='" & aqjh_name & "'"
		
		connt.execute "update 妓男 set 美貌度=美貌度-10 where id=" & id
	case 3
		mess=aqjh_name &"在人妖馆和"&mingji&"谈笑风生，居然能让"&mingji&"自愿给"&aqjh_name&"舔了舔小脚指头，"&aqjh_name&"道德上升100点，内力增加"& int(meimao/2) &"点，临走时满意地放下"& yin &"两银子，"& mingji &"得到基中的一半，美貌度上升20点！"
		conn.execute "update 用户 set 银两=银两-" & yin & ",道德=道德+100,体力=体力-500,内力=" & newnl & ",操作时间=now() where 姓名='" & aqjh_name & "'"
		
		connt.execute "update 妓男 set 美貌度=美貌度+20 where id=" & id
	case 4
		mess=aqjh_name &"不知是何居心，居然想把"&mingji&"真的阉掉，被馆内保镖痛打一顿，体力下降10000，内力下降"& int(meimao/3) &"，道德下降100，魅力下降100！"
		conn.execute "update 用户 set 体力=体力-10000,内力=内力-int(meimao/3),道德=道德-100,魅力=魅力-100,银两=银两-" & yin & ",操作时间=now() where 姓名='" & aqjh_name & "'"
		
end select
says="<font color=#ff0000><b>【小道消息】</b></font>"&mess			'聊天数据
says=replace(says,"'","\'")
