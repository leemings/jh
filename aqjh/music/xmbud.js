...
function wmpStop(){
 ...
 if(blnStatusBar){window.status=('NwLife Player');return true;}
}
...
// 12(MediaOpening) 13(MediaOpen) 20(MediaWaiting) 21(OpeningUnknownURL)
 ...
 if(f==8){capText.innerHTML="Nwlife Player";}
 if(f==13){
	...
	strMmInfo= "Media (Title) : " + strTitle + " (Nwlife Player:" + strType +")" + "\n"
	...
	strMmInfo= strMmInfo + "Media (Duration) : " +strDur +"\n\n";
        strMmInfo= strMmInfo + "http://www.Archong.com \n";
	strMmInfo= strMmInfo + "Archong.com 2000-2010\n";
...
function waitMsg(){
 ...
 if(intExobudStat ==2){disp1.innerHTML="Nwlife Player";}
 if(blnStatusBar){
	if(intExobudStat ==1){window.status=('Nwlife Player');return true;}
	if(intExobudStat ==2){window.status=('Nwlife Player'); return true;}
 }
}
...
function vizExobud(){
  window.open("http://www.nwlife.net","vizExobud");
}