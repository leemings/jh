...
if rs("门派")="官府" and rs("身份")>"6" then
UID=rs("ID")
sql="select * from 用户 where 状态='牢' or 状态='狱' and 门派<>'官府' and id=" & id
set rs=conn.execute(sql)
...
says="<font color=red><b>" & rs("姓名") & "被无罪释放</b>(执行官ID="&UID&")</font>"			'聊天数据
says=replace(says,"'","\'")
