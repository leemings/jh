...
<tr>
<td width="100%"><font color="#ffffff" size="2">50位法器最多的侠客(非官府)</font></td>
</tr>
</table>
<%
dim sql
dim rs
dim filename
sql="select * from 用户 where 门派<>'官府' and DateDiff('m',cdate(lasttime),now())=0 order by w10 desc"
...
bordercolordark="#FFFFFF" cellpadding="0" bordercolor="#000000">
    <tr> 
      ...
      <td align="center" height="16" bgcolor="#336633" width="9%"><font color="#FFFFFF">法器</font></td>
    </tr>
    <%do while not rs.eof%>
    <tr bgcolor="#85C2E0" onmouseout="this.bgColor='#85C2E0';"onmouseover="this.bgColor='#DFEFFF';"> 
      ...
      <td align="center" width="9%"><font color=ff0000><%=rs("w10")%></font> 
      </td>
    </tr>
    <%
rs.movenext
