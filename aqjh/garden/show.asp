...
case 8
rs.open "select 银两,hua,w7 from 用户 where 姓名='"&name&"'",conn,2,2
if rs("银两")<10000 then
Response.Write "<script language=JavaScript>{alert('提示：你没有 10000 的银两，操作失败！');parent.gg();}</script>"
rs.close
set rs=nothing
...
case 5
rs.open "select hua ,w7 from 用户 where 姓名='"&name&"'",conn,2,2
...
conn.execute "update 用户 set hua='"&ghuasj&"',w7='"&zstemp&"' where 姓名='" & name &"'"
Response.Write "<script language=JavaScript>{parent.gg();alert('提示：您收获了您种的花 ["&huaname&"] ，快送给心爱的人分享！');}</script>"
seedsm=fhuasj(0)
Response.Write "<script language=JavaScript>{parent.gs("&seedsm&");parent.gg();}</script>"
case 6
