...
sql1="select * from 花园风云榜 where 参赛人='"&name&"'"
rs.open sql1,connt,1,1
if rs.recordcount=0 then
 Response.Write "<script Language=JavaScript>alert('提示：此人未参与花园大赛！');window.close();</script>"
 Response.End
else
 rs.close
 sql2="select * from 花园投票者 where 投票ID="&aqjh_id
 rs.open sql2,connt,1,1
 if rs.recordcount<>0 then
   Response.Write "<script Language=JavaScript>alert('提示：不得重复投票！');window.close();</script>"
   Response.End
 else
   sql3="update 花园风云榜 set 票数=票数+1 where 参赛人='"&name&"'"
   connt.execute(sql3)
   sql4="INSERT INTO 花园投票者 (投票ID,姓名) VALUES ("&aqjh_id&",'"&aqjh_name&"')"
   connt.execute(sql4)
