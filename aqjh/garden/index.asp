...
dj=rs("等级")
rs.close
rs.open "Select * from 花园投票者 where 投票ID="&aqjh_id,connt,3,3
...
rs.close
sql="Select count(*) As 数量 from 花园投票者"
set tmprs=connt.execute(sql)
ytrs=tmprs("数量")
tmprs.close
set tmprs=nothing
sql="Select count(*) As 数量 from 花园风云榜"
set tmprs=connt.execute(sql)
hxr=tmprs("数量")
tmprs.close
set tmprs=nothing
sql="select * from 花园风云榜 order by 票数 desc"
...
<table width=500 border="0" align=center cellspacing="0" bordercolor="#FF0000">
        <tr bordercolor="#FFFFFF">
          <td align=center><font color=red><b>爱  情  江 湖 花 园 大 赛 投 票 处</b></font><BR><BR></td>
        </tr>
      </table>
...
<center><FONT color=#0000ff>&copy; 版权所有 2005-2006 </FONT><A href="http://www.7758530.com/" target=_blank><FONT color=#0000ff>爱情江湖网</FONT></A>
</body></html>