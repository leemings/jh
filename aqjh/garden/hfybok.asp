...
if rs.recordcount=0 then
 ...
 Response.End
  ...
  select case rs("名次")
  case 1
    sql="update 用户 set 花魁=花魁+100,金币=金币+500,会员金卡=会员金卡+10 where 姓名='"&name&"'"
    mess="荣获花园大赛<font color=red>一等奖</font>，官府奖励金币500个，现金(会员金卡)10元，站长亲自颁奖！恭喜了！"
  case 2
    sql="update 用户 set 花魁=花魁+50,金币=金币+300,会员金卡=会员金卡+5 where 姓名='"&name&"'"
    mess="荣获花园大赛<font color=red>二等奖</font>，官府奖励金币300个，现金(会员金卡)5元，站长亲自颁奖！恭喜了！"
  case 3
    sql="update 用户 set 花魁=花魁+20,金币=金币+100,会员金卡=会员金卡+3 where 姓名='"&name&"'"
    mess="荣获花园大赛<font color=red>三等奖</font>，官府奖励金币100个，现金(会员金卡)3元，站长亲自颁奖！恭喜了！"
  case else
     Response.Write "<script Language=JavaScript>alert('提示：奖品出错！');window.close();</script>"
     Response.End
  end select
  sql2="update 花园风云榜 set 领奖=true where 参赛人='"&name&"'"
  conn.execute(sql)
  connt.execute(sql2)
end if
rs.close
sql="Select hua from 用户 where 姓名='"&aqjh_name&"'"
...
conn.execute "update 用户 set hua='"&myhua_ok&"' where 姓名='"&aqjh_name&"'"
call showchat ("<bgsound src=../chat/wav/hkjr.mp3 loop=2><font color=red>【种花大赛】</font><img src=pic/hk.gif>"&aqjh_name&mess)
Response.Write "<script Language=JavaScript>alert('提示：恭喜你已经领取了花园大赛奖品');window.close();</script>"
Response.End
%>