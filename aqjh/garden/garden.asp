...
Sub Msg (v)
 ...
 Response.End
End Sub
If Session("aqjh_name") = "" Then Msg "您尚未登录，不能赏花。"
name = Session("aqjh_name")
If name = "" Then Msg "您尚未登录，不能赏花。"
...
rs.open "SELECT 银两,金币,操作时间,道德,魅力,体力,内力 FROM 用户 WHERE 姓名='"& session("aqjh_name") &"'",conn,1,3
sj=DateDiff("s",rs("操作时间"),now())
if sj<300 then
s=300-sj
Response.Write "<script Language=Javascript>alert('赏花要300秒一次，请等"& s &"秒,可别累着！');</script>"
response.end
end if
if rs("银两")<100000 or rs("金币")<1 then
	...
	conn.close
	set conn=nothing
	response.write "身上没100000银两或金币不足1个，不能赏花！"
	response.end
end if
if rs("道德")<1000 or rs("魅力")<5000 then
	...
	conn.close
	set conn=nothing
	Response.Write "道德不够1000或魅力不够5000，不能赏花！"
	Response.End
end if
...
set conn=nothing
%>
<HTML><HEAD><TITLE><%=name%>的小花园</TITLE>
<script>
window.moveTo(40,30);
</script>
...
function go(i,fc,ft,fa){
	//if(i>24||fc==''||fc==''||fc==''){
//	flowersact.innerHTML="网络有问题，请刷新叶面";
//	return;
//	}
...
function highLight(id,fx,fy) {
	if(top.isMember==0&&id>8) {
		flowersact.innerHTML="您的花园是3X3大小，五级以上才有5X5的大花园";
		return ;}
	if(flag){
		noneLight();
		theid=parseInt(id);
...
//alert("此处不存在鲜花无法操作!");return;
		...
		if(confirm("您确定要将花采下换为金币，还是将花变成3颗种子？\n点确定是采下，点取消是变种子。")) {
...
function count() {
   ...
   if (random>0.3&&random<0.4&&dialogcount>0)
    ...
    if (random>0.58&&random<0.7&&dialogcount>0)
		dialogbox.innerHTML="如果等级大于等于五级，就可以种满整个花园的花，……";
...
<TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
  <TBODY>
  <TR>
    <TD width="50%" height=44><IMG onmouseup="changeImage(button5, 'images/button5.GIF');" onmousedown="" id=button5 title=播种 onclick="" height=44 src="images/button5.gif"  width=42></TD>
    <TD height=44><IMG onmouseup="changeImage(button4, 'images/button4.GIF');" onmousedown="" id=button4 title=剪枝 onclick="" height=44 src="images/button4.gif"  width=42></TD>
  </TR>
  <TR>
    <TD height=44><IMG onmouseup="changeImage(button3, 'images/button3.GIF');" onmousedown="" id=button3 title=浇水 onclick="" height=44 src="images/button3.gif" width=42></TD>
    <TD height=44><IMG onmouseup="changeImage(button2, 'images/button2.GIF');" onmousedown="" id=button2 title=施肥 onclick="" height=44 src="images/button2.gif" width=42></TD>
  </TR>
  <TR>
    <TD height=44><IMG onmouseup="changeImage(button1, 'images/button1.GIF');" onmousedown="" id=button1 title=除虫 onclick="" height=44 src="images/button1.gif" width=42></TD>
    <TD height=44><IMG onmouseup="changeImage(button6, 'images/button6.GIF');" onmousedown="" id=button6 title=收获 onclick="" height=44 src="images/button6.gif" width=42></TD>
   </TR>
  <TR>
    <TD colSpan=2 height=44> <DIV align=center><%if Session("aqjh_name")=name then%><A onmouseup="changeImage('button7', 'images/button7.gif');" onmousedown="" title=第一次种花必须开垦花园,如果已经种了花，就会将你的花园重新犁一遍，现在种的花都会犁掉。 onclick=""  href="#" target=_top><IMG id=button7 height=44 alt="开 垦 花 园" src="images/button7.gif"  width=55 border=0></A><%end if%></DIV> </TD>
   </TR>
   </TBODY>
   </TABLE>
   </DIV>
...
conn.open Application("aqjh_usermdb")
aqjh_grade=Session("aqjh_grade")
rs.open "select 会员,hua from 用户 where 姓名='"&name&"'",conn,2,2
if rs("hua")<>"" then
else
