...
if rs("银两")<1000000 then
		Response.Write "<script language=javascript>alert('抱歉你的银两不够');history.back();</script>"
		rs.close
		set rs=nothing
		conn.close
		set conn=nothing
...
conn.execute "update 用户 set 体力=体力-10000,内力=内力-5000,银两=银两-1000000,武功=武功+5000,操作时间=now() where 姓名='"&aqjh_name&"'"
rs.close
set rs=nothing
conn.close
set conn=nothing
message="" & my & "辛辛苦苦学了一天，你获得武功5000！消耗体力10000，内力5000，银两100万！"
end if
%>
