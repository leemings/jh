...
if Weekday(date())<>7 then
	Response.Write "<script Language=Javascript>alert('对不起，周六礼物只能在星期六领取！');location.href = 'javascript:history.go(-1)';</script>"
	response.end
end if
Set conn=Server.CreateObject("ADODB.CONNECTION")
Set rs=Server.CreateObject("ADODB.RecordSet")
conn.open Application("aqjh_usermdb")
rs.open "SELECT id,w1,mvalue,等级,会员等级 FROM 用户 WHERE 姓名='"&session("aqjh_name")&"' and 密码='"&psw&"'",conn
...
if rs("mvalue")<=5000 then
	...
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('月积分在5000以上的才可以领取物品！');location.href = 'javascript:history.go(-1)';</script>"
	response.end
end if
if rs("等级")<=60 then
	rs.close
	...
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('为防止作弊，只有等级在60级以上的才可以领取物品！');location.href = 'javascript:history.go(-1)';</script>"
	response.end
end if
...
select case r
	case 1 '药王金丹
		n=int(rnd*5)+6
		temp=add(wpdate,"药王金丹",100)
                sql="update 用户 set w1='"&temp&"' where id="&myid
		lw="药王金丹100个"
	case 2 '药王圣水
		n=int(rnd*5)+6
		temp=add(wpdate,"药王圣水",100)
                sql="update 用户 set w1='"&temp&"' where id="&myid
		lw="药王圣水100个"
