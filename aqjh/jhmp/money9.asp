...
if rs("会员等级")<>0  then
Response.Write "<script language=JavaScript>{alert('你是等级会员,不应该来这种地方的!');location.href = 'javascript:history.back()';}</script>"
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
...
if Session("aqjh_grade")>=5 then
 Response.Write "<script language=JavaScript>{alert('掌门和官府是不应该来这种地方的!');location.href = 'javascript:history.back()';}</script>"
 Response.End
end if
pai=rs("门派")
rs.close
...
Set rs=Server.CreateObject("ADODB.RecordSet")
conn.open Application("aqjh_usermdb")
rs.open "select 等级 FROM 用户 WHERE 姓名='" & aqjh_name &"'" ,conn,2,2
dj=rs("等级")
if dj<20 then
Response.Write "<script language=JavaScript>{alert('你战斗等级不够20级不能进来领取金币!');location.href = 'javascript:history.back()';}</script>"
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
...
if trim(rs("身份"))="掌门" then
	...
	renshu=tmprs("数量")
	'算钱数
	money=cint(renshu)*100000+jsren*50000+dj*50000+100000
	conn.execute("update 用户 set 银两=银两+"&money&",领钱=now() where 姓名='"&aqjh_name&"'")
else
	'个人按会员乘10万
	money=jsren*50000+dj*50000+500000+1000000-lp*50000+zs*200000
	conn.execute("update 用户 set 银两=银两+"&money&",领钱=now() where 姓名='"&aqjh_name&"'")
end if
rs.close
rs.open "Select * from 用户 where 姓名='"&aqjh_name&"'",conn
...
<font color="#0000FF">弟子：</font>介绍人数x5万+战斗等级x5万+100万</p>
<font color="#0000FF">转生：</font>（总工资）+（转生次数*200000）<br>
<font color="#0000FF">离派次数：</font>（总工资）-（离派次数*500000）</p>
<p align="center"><b><font color="#FF00FF">战斗等级20级以上另外送金币<img border="0" src="Jb.GIF">1个作为奖励。</font></b></p>
<p align="center">&nbsp;</p>
<p align="center">
