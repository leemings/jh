...
if rs("会员等级")=0 then
		...
		conn.close
		set conn=nothing
		Response.Write "<script language=JavaScript>{alert('你不是等级制会员，请回吧!');location.href = 'javascript:history.back()';}</script>"
		response.end
...
if rs("会员等级")<>3  then
Response.Write "<script language=JavaScript>{alert('你不是三级会员啊!');location.href = 'javascript:history.back()';}</script>"
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
...
if Session("aqjh_grade")>=6 then
 Response.Write "<script language=JavaScript>{alert('这不是官府应该来的地方！');location.href = 'javascript:history.back()';}</script>"
 Response.End
end if
pai=rs("门派")
rs.close
...
if trim(rs("身份"))="掌门" then
	...
	renshu=tmprs("数量")
	'算钱数
	money=((hy+1)*500000+cint(renshu)*50000+jsren*50000+dj*500000+500000)*2+1000000
	conn.execute("update 用户 set 工资小点=工资小点+1,银两=银两+"&money&",领钱=now() where 姓名='"&aqjh_name&"'")
else
	'个人按会员乘10万
	money=((rs("会员等级")+1)*500000+jsren*50000+dj*50000+500000)*2+1000000-lp*500000+zs*200000
	conn.execute("update 用户 set 工资小点=工资小点+1,银两=银两+"&money&",领钱=now() where 姓名='"&aqjh_name&"'")
end if
rs.close
rs.open "Select * from 用户 where 姓名='"&aqjh_name&"'",conn
conn.execute "update 用户 set 会员金卡=会员金卡+3 where 姓名='"&aqjh_name&"'"
%>
<html>
<head>
<title>工资领取处</title>
...
set rs=nothing
set conn=nothing
says="<font color=red>【领取工资】["&aqjh_name&"]</font><font color=blue>今天刚领了<font color=red>"&money&"</font>两的工资，3级会员另外领取<font color=red>3块</font>金卡,大家还不快去,等一下就没有了!</font>"
call showchat(says)
%>
<br>
</p>
...
<font color="#0000FF">等级制会员掌门：</font>（会员等级x50万+弟子数x5万+介绍人数x50万+战斗等级x5万）x2+（100万会员费）<br>
<font color="#0000FF">等级制会员弟子：</font>（会员等级x50万+介绍人数x5万+战斗等级x5万）x2+（100万会员费）</p>
<font color="#0000FF">转生：</font>（总工资）+（转生次数*200000）<br>
<font color="#0000FF">离派次数：</font>（总工资）-（离派次数*500000）</p>
<p align="center"><b><font color="#FF00FF">三级等级制会员另外领取金卡<img border="0" src="jk.GIF">3元！</font></b></p>
<p align="center">&nbsp;</p>
<p align="center">
