...
if trim(rs("身份"))="掌门" then
	...
	renshu=tmprs("数量")
	'算钱数
	money=(cint(renshu)*100000+jsren*50000+dj*50000+500000)*2+1000000-lp*500000+zs*200000
	conn.execute("update 用户 set 工资小点=工资小点+1,银两=银两+"&money&",领钱=now() where 姓名='"&aqjh_name&"'")
else
	'个人按会员乘10万
	money=(rs("会员等级")+1)*100000+jsren*50000+dj*5000+500000
	conn.execute("update 用户 set 工资小点=工资小点+1,银两=银两+"&money&",领钱=now() where 姓名='"&aqjh_name&"'")
end if
rs.close
rs.open "Select * from 用户 where 姓名='"&aqjh_name&"'",conn
...
set rs=nothing
set conn=nothing
says="<font color=red>【领取工资】["&aqjh_name&"]</font><font color=blue>今天刚领了<font color=red>"&money&"</font>两的工资，掌门另外领取<font color=red>2枚</font>金币,大家还不快去,等一下就没有了!</font>"
call showchat(says)
%>
<br>
</p>
...
<font color="#0000FF">掌门</font>：（弟子数x10万+介绍人数x5万+战斗等级x5万）x2+（100万掌门费）
