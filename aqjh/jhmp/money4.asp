...
if rs("会员等级")<>4  then
Response.Write "<script language=JavaScript>{alert('你不是四级会员啊!');location.href = 'javascript:history.back()';}</script>"
rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
...
if trim(rs("身份"))="掌门" then
	...
	renshu=tmprs("数量")
	'算钱数
	money=((hy+1)*500000+cint(renshu)*50000+jsren*50000+dj*50000+500000)*2+1000000
...
rs.close
rs.open "Select * from 用户 where 姓名='"&aqjh_name&"'",conn
conn.execute "update 用户 set 会员金卡=会员金卡+4 where 姓名='"&aqjh_name&"'"
%>
<html>
<head>
<title>工资领取处</title>
...
set rs=nothing
set conn=nothing
says="<font color=red>【领取工资】["&aqjh_name&"]</font><font color=blue>今天刚领了<font color=red>"&money&"</font>两的工资，4级会员另外领取<font color=red>4块</font>金卡,大家还不快去,等一下就没有了!</font>"
call showchat(says)
%>
<br>
</p>
...
<font color="#0000FF">等级制会员掌门：</font>（会员等级x50万+弟子数x5万+介绍人数x5万+战斗等级x5万）x2+（100万会员费）<br>
...
<p align="center"><b><font color="#FF00FF">四级等级制会员另外领取金卡<img border="0" src="jk.GIF">4元！</font></b></p>
<p align="center">&nbsp;</p>
<p align="center">
