...
if rs("会员")=true and clng(DateDiff("d",now(),rs("会员结束")))>0 then
	...
	pdstr="非泡点会员"
		...
		conn.close
		set conn=nothing
		Response.Write "<script language=JavaScript>{alert('你不是泡点制会员，请止步!');location.href = 'javascript:history.back()';}</script>"
		response.end
end if
...
if  DateDiff("d",rs("薪水"),now())=0 then
	rs.close
	set rs=nothing
	conn.close
	set conn=nothing
...
if trim(rs("身份"))="掌门" then
	...
	conn.execute("update 用户 set 银两=银两+"&money&",薪水=now() where 姓名='"&aqjh_name&"'")
else
	'个人按会员乘10万
...
set rs=nothing
set conn=nothing
says="<font color=red>【领取工资】["&aqjh_name&"]</font><font color=blue>今天刚领了<font color=red>"&money&"</font>两的工资,泡点会员另外领取<font color=red>2块</font>金卡,大家还不快去,等一下就没有了!</font>"
call showchat(says)
%>
<br>
</p>
...
<font color="#0000FF">泡点制会员掌门：</font>（会员等级x50万+弟子数x5万+介绍人数x5万+战斗等级x5万）x2+（100万会员费）<br>
<font color="#0000FF">泡点制会员弟子：</font>（会员等级x50万+介绍人数x5万+战斗等级x5万）x2+（100万会员费）</p>
<font color="#0000FF">转生：</font>（总工资）+（转生次数*200000）<br>
<font color="#0000FF">离派次数：</font>（总工资）-（离派次数*500000）</p>
<p align="center"><b><font color="#FF00FF">泡点制会员另外领取金卡<img border="0" src="jk.GIF">2元！</font></b></p>
<p align="center">&nbsp;</p>
<p align="center">
