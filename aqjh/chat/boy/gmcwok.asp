...
if newname="" or oldname="" or oldname=newname then
	Response.Write "<script language=JavaScript>{alert('提示：小孩名不能为空，也不能与原名相同!');}</script>"
	Response.End 
end if
namelen=0
...
for i=0 to ubound(bad)-1
	if InStr(LCase(newname),bad(i))<>0 then 
		Response.Write "<script language=JavaScript>{alert('提示：小孩名不雅，重新输入！');}</script>"
		Response.end
	end if
next
...
rs.open "select boy,配偶 from 用户 where 姓名='"&aqjh_name&"'",conn,2,2
zz=rs("配偶")
...
conn.close
set conn=nothing
says="<font color=#ff0000><b>【小孩改名】</b></font>经过公安厅厅长认可，##把给自己的小孩改名为[<font color=red><b>"&newname&"</b></font>]成功……"
...
Application.UnLock()
End Sub
Response.Write "<script Language=Javascript>alert('提示：小孩名更改成["&newname&"]成功!');parent.f3.location.href = 'boy.asp';</script>"
%>
