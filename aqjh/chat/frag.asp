...
rs.close
rs.open "select 粪库,内力,银两,性别 FROM 用户 WHERE 姓名='"&aqjh_name&"'",conn
tosex=rs("性别")
Response.Write "<script language=JavaScript>{alert('要记得常清理粪库喔要不然粪库堆到5000点会死喔！');}</script>"
frag="【夜香师】：<font color=red>※"& fromname &"※</font>正在替，<font color=red>※"& aqjh_name &"※</font>清理粪库真是又臭又脏.这年头2万银两真不好赚^_^<font color=red>※"& fromname &"※</font>清理完立刻洗澡去.."
	   	 conn.execute "update 用户 set  银两=银两-500000,粪库=0,内力=内力-1000 where 姓名='"&aqjh_name&"'"
	     conn.execute "update 用户 set  银两=银两+500000,内力=内力+1000 where 姓名='"&fromname&"'"
set rs=nothing
conn.close
set conn=nothing
Application.Lock
