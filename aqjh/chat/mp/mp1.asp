...
Response.Write "<script Language=Javascript>alert('提示：门派大战请于每周日晚上18-22点进行！每周比赛结束后管理员会来查看那间门派战斗力剩下最多,战斗力最多者将可到总站免费半年会员.！');parent.f2.document.af.mdsx.checked=true;parent.m.location.reload();</script>"
	Response.End
end if
	sql="select 体力,门派,武功,内力,防御,攻击,金币,等级,身份 from 用户 where 姓名='" & aqjh_name &"'"
	...
	set conn=nothing
     ...
     if mp="官府" then
       Response.Write "<script language=JavaScript>{alert('您是官府的人，怎么能跟江湖门派结怨了！');}</script>"
       rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
       Response.End
         ...
         if zhan1<=1000 then
       Response.Write "<script language=JavaScript>{alert('您的战斗力怎么低,怎么替门派出战啊！');}</script>"
       rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
...
rs.close
 ...
 if topai="官府" then
	...
	set conn=nothing	
       ...
       Response.Write "<script language=JavaScript>{alert('他们门派好像没有和你的门派开战呀,如果想开战向与掌门请求');}</script>"
       rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
	      ...
	      kills=int(rnd*99)+1
	            	...
	            	conn.execute"update p set s=s-"& kills3 & " where a='" & topai & "'"
			         ...
			         if shengming-kills3<=0 then
                          ...
                          call boot(to1,"门派大战，操作者："&aqjh_name&",["&mp&"]"&fn1)
                          conn.execute"insert into 人命(死者,时间,凶手,死因) values ('" & topai & "',now(),'" &aqjh_name & "','被门派内奸出卖')"
                         

			              menattack=e&"杀伤"&to1&"战斗力为<b><font color=red>"&kills&"</font></b>,杀伤"&topai&"门派战斗力为<b><font color=red>"&kills3&"</font></b>,并最终使改门派覆灭,"& aqjh_name &"作为卧底冲锋陷阵,道德上升1000,魅力上升1000,金币增加<b><font color=red>10</font></b>!"
                          else 
                        ...
                        menattack=e&"杀伤"&to1&"战斗力为<b><font color=red>"&kills&"</font></b>,杀伤对方"&topai&"门派战斗力为<b><font color=red>"&kills3&"</font></b>,并最终使改门派覆灭,"& aqjh_name &"为门派冲锋陷阵,道德上升1000,魅力上升1000,金币增加<b><font color=red>10</font></b>!"
						 end if
                             rs.close
                             set rs=nothing	
	                      conn.close
                        ...
                        menattack=e&"杀伤"&to1&"战斗力为<b><font color=red>"&kills&"</font></b>,杀伤对方"&topai&"门派战斗力为<b><font color=red>"&kills3&"</font></b>,"& aqjh_name &"为门派冲锋陷阵,道德上升2!"
