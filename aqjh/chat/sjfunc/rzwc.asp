...
rs.open "select 金币,任务,银两,体力,法力,知质,会员金卡,精神,w6,木,操作时间 from 用户 where 姓名='"&aqjh_name&"'",conn
...
if aqjh_jhdj<60 then
	...
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('江湖等级需要超过60级才可提交任务！');window.close();</script>"
	Response.End
end if
...
if rs("木")<2000 then
	...
	conn.close
	set conn=nothing
 	Response.Write "<script language=JavaScript>{alert('你没有2000木属性无法完成任务！');location.href = 'rwbz.asp';}</script>"
	response.end
end if
if trim(rs("任务"))<>"人族任务" then
	rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('提示：您的任务不是人族任务，所以您不能进行人族任务操作！！');window.close();</script>"
	response.end
end if
...
rs.close
	...
	conn.close
	set conn=nothing
Response.Write "<script language=JavaScript>{alert('精神力需要5点！作为人类自身的精神力是必须的..');window.close();}</script>"
Response.End
end if
if rs("法力")<1000 then
rs.close
...
conn.execute "update 用户 set 木=木-2000 where 姓名='"& aqjh_name &"'"
conn.execute "update 用户 set 精神=精神-5 where 姓名='"& aqjh_name &"'"
randomize timer
r=int(rnd*10)+1
jinbi=0
jinka=0
...
case 1
	...
	erase js
case 2
	mess=""& aqjh_name &"完成任务得到金币30个，金卡10元"
	sql="update 用户 set 操作时间=now(),金币=金币+30,会员金卡=会员金卡+10 where 姓名='"&aqjh_name&"'"
case 3
	mess=""& aqjh_name &"完成任务得到金币31个"
	sql="update 用户 set 操作时间=now(),金币=金币+31 where 姓名='"&aqjh_name&"'"
case 4
	mess=""& aqjh_name &"完成任务得到金币32个"
	sql="update 用户 set 操作时间=now(),金币=金币+32 where 姓名='"&aqjh_name&"'"
case 5
	mess=""& aqjh_name &"完成任务得到金币33个"
	sql="update 用户 set 操作时间=now(),金币=金币+33 where 姓名='"&aqjh_name&"'"
case 6
	mess=""& aqjh_name &"完成任务金卡增加2元"
	sql="update 用户 set 操作时间=now(),会员金卡=会员金卡+2 where 姓名='"&aqjh_name&"'"
	case 7
	mess=""& aqjh_name &"完成任务金卡增加3元"
	sql="update 用户 set 操作时间=now(),会员金卡=会员金卡+3 where 姓名='"&aqjh_name&"'"
	case 8
	mess=""& aqjh_name &"完成任务金卡增加5元"
	sql="update 用户 set 操作时间=now(),会员金卡=会员金卡+5 where 姓名='"&aqjh_name&"'"
	case 9
	mess=""& aqjh_name &"完成任务增加江湖存点增加8660，有可能升级的哦！"
	sql="update 用户 set 操作时间=now(),allvalue=allvalue+8660 where 姓名='"&aqjh_name&"'"
        case 10
        rs.close
