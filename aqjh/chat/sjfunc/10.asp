...
rs.close
rs.open "select 门派,保护,杀人数,grade,w2,死亡时间,宝物,杀人时间 from 用户 where 姓名='" & aqjh_name &"'",conn,2,2
if rs("门派")="出家" and aqjh_grade<10 then
	rs.close
	set rs=nothing
	conn.close
...
if sj<30 then
	...
	conn.close
	set conn=nothing
        call mess ("提示：你刚刚杀了一个人，得30分钟后再可以杀人了！",1)
end if
sj=DateDiff("n",rs("死亡时间"),now())
if sj<5 then
		rs.close
...
if rs("杀人数")>=int(Application("aqjh_killman")) and aqjh_grade<10 then
	...
	conn.close
	set conn=nothing
	call mess ("提示：你作恶太多，超过江湖杀人上限"& Application("aqjh_killman") &"不能再下毒了！",1)
end if
if rs("保护")=True then
	rs.close
	set rs=nothing
...
conn.execute "update 用户 set w2='"&duyao&"' where  姓名='"&aqjh_name&"'"
rs.close
'取出毒药杀伤力
...
tl=abs(rs("e"))*wusl
'用户
conn.execute "update 用户 set 道德=道德-20,体力=体力-" & int(tl/4) & " where 姓名='" & aqjh_name &"'"
conn.execute "update 用户 set 内力=内力-" & nl & ",体力=体力-" & tl & " where 姓名='" & to1 &"'"
rs.close
over=""
rs.open "select 体力 from 用户 where 姓名='" & aqjh_name &"'",conn,2,2
if rs("体力")<-100 then
	conn.execute "update 用户 set 状态='死',事件原因='"&aqjh_name&"|下毒:不小心自己中毒了…',死亡时间=now() where 姓名='" & aqjh_name &"'"
	over="##不小心自己也中了剧毒，啊的一声，倒下了…"
	call boot(aqjh_name,"下毒，不小心自己中毒了！")
end if
rs.close
rs.open "select 姓名,体力,宝物,杀人时间 from 用户 where 姓名='" & to1 &"'",conn,2,2
to1=rs("姓名")
if rs("体力")>-100 then
	xiadu="##<bgsound src=wav/anqi.wav loop=1>向%%下["&zswupin&"]共:"&wusl&"个,使%%的内力:-<font color=red>" & nl & "</font>点,体力:-<font color=red>" & tl & "</font>点!环境污染"& aqjh_name &"体力:-<font color=red>"& int((tl)/4 ) &"</font>点!"&over
	rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
...
if rs("宝物")=Application("aqjh_baowuname") then
	...
	xiadu="##把%%的宝物:"& Application("aqjh_baowuname") &"抢走，因得到此宝。江湖宝物需要进行修练才可以得到更多的东西！"&over
	rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
	...
	xiadu=aqjh_name & "<bgsound src=wav/daipu.wav loop=1>向" & to1 & "偷偷下了毒药["&zswupin&"]共:"&wusl&"个,使" & to1 & "的内力:-" & nl & "体力:-" & tl & "," & to1 & "咽气前说道“查出凶手,为我报仇……”"&over
	conn.execute "update 用户 set 杀人时间=now(),状态='死',事件原因='"&aqjh_name&"|下毒:"&zswupin&"',死亡时间=now() where 姓名='" & to1 &"'"
'记录
...
conn.close
set conn=nothing
call boot(to1,"下毒，操作者："&aqjh_name&","&zswupin)
end function
%>