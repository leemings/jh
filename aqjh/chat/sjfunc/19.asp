...
if chatinfo(0)<>"聊天大厅" then
 Response.Write "<script language=javascript>{alert('提示：拍卖物品请去大厅，这里没人在！');}</script>"
 Response.End
end if
...
if to1=aqjh_name or to1=Application("aqjh_automanname") then to1="大家"
if InStr(Application("aqjh_mai"),"|")<>0 then
	temp=split(Application("aqjh_mai"),"|")
...
if zt(1)<>"银两" then 
	Response.Write "<script language=JavaScript>{alert('提示：币制只能为银两！');}</script>"
	Response.End 
end if
lb=zt(2)
if lb<>"w1" and lb<>"w2" and lb<>"w4" and lb<>"w5" and lb<>"w6" and lb<>"w7" and lb<>"w8" and lb<>"w10" then
...
rs.open "select "&lb&" from 用户 where 姓名='"&aqjh_name&"'",conn,2,2
...
Application("aqjh_mai")=aqjh_name&"|"&zt(0)&"|"&zt(1)&"|"&lb&"|"&zt(3)&"|"&zt(4)&"|"&now()&"|"&to1
Application.UnLock
randomize()
