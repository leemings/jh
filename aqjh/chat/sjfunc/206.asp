...
rs.open "select 职业,内力,银两 FROM 用户 WHERE 姓名='" & aqjh_name &"'",conn,2,2
...
if trim(rs("职业"))<>"倒夜香" then
	rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('提示：您的职业不是倒夜香师，请不要作弊！！');window.close();</script>"
	response.end
end if
if rs("内力")<1000 then
	rs.close
...
if rs("银两")<500000 then
	...
	conn.close
	set conn=nothing
	Response.Write "<script language=JavaScript>{alert('提示：对方没有50万块钱，不能帮他倒夜香！');}</script>"
	Response.End
end if
if rs("内力")<1000 then
	rs.close
...
frag="倒夜香师<img src='pic/dz621.gif' width='40' height='55'>[##]替{%%}倒粪库：倒一次粪库收费2万银两内力1000喔<input style='font-size: 9pt; background-color: #99ccff; border-style: ridge' type=button value='立刻解决' onClick=javascript:;tongyi"&regjm&".disabled=1;tongyi"&regjm&".disabled=1;window.open('frag.asp?fromname=" & aqjh_name &"&toname="&to1&"','d') name=tongyi"&regjm&">" 
