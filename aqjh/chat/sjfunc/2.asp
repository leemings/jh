...
if rs("grade")<aqjh_grade or aqjh_name=Application("aqjh_user") then
	conn.execute "update 用户 set 登录=now()+0.02,状态='点穴',事件原因='"&aqjh_name&" 点穴:"&fn1&"' where 姓名='" & to1 &"'"
	dian="##对%%使用了点穴术，%%呆呆地不动了……需28分钟才可以上线……原因："&fn1
	call boot(to1,"点穴，操作者："&aqjh_name&","&fn1)
else
	dian="##对%%使用了点穴术，可是他是官府中高级管理员,你对他没有办法…"
end if
'记录
conn.execute "insert into l(a,b,c,d,e) values (now(),'"& aqjh_name &"','"& to1 &"','点穴','"& fn1 & "')"
rs.close
set rs=nothing	
conn.close
set conn=nothing
