...
if aqjh_jhdj<50 then
	Response.Write "<script language=JavaScript>{alert('提示：当男贩的等级最少得50级才行！');}</script>"
	Response.End
end if
...
if rs("职业")<>"男贩" then
	...
	conn.close
	set conn=nothing
	Response.Write "<script language=JavaScript>{alert('提示：你的职业不是男贩，不能操作！');}</script>"
	Response.End
end if
...
sql="select * from 妓男 where 妓男ID="&toid
...
select case r
	case 1
		conn.execute "update 用户 set 银两=银两-50000000,道德=道德-500,魅力=魅力-500 where 姓名='"&aqjh_name&"'"
	...
	case 2
		...
		sql="insert into 妓男(妓男ID,姓名,美貌度) values ("& toid &",'" & to1 & "'," & meimao & ")"
set rs1=connt.execute(sql)
		fmrk=aqjh_name & "的嘴皮子真是好用，连蒙带骗的就把"& to1 &"卖到人妖馆了，得到好处费50万，道德下降1000。" 
	case 3
	...
	case 4
		...
		fmrk=aqjh_name & "想把"& to1 &"骗到人妖馆没成功，但却骗到她所有现金的一半"& yinliang &","& aqjh_name &"道德下降500点，魅力下降200点。"
	case 5
		conn.execute "update 用户 set 银两=银两-50000000,道德=道德-500,魅力=魅力-500 where 姓名='"&aqjh_name&"'"
