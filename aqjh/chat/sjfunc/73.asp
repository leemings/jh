...
if chatinfo(5)<>0 then
	Response.Write "<script language=JavaScript>{alert('提示：在["&chatinfo(0)&"]房间不可以宠物攻击！');}</script>"
	Response.End
end if
...
rs.close
rs.open "select 门派,grade,保护,杀人数,cw from 用户 where  姓名='"&aqjh_name&"'",conn,2,2
if rs("门派")="出家" and aqjh_grade<10 then
	rs.close
	set rs=nothing	
	conn.close
...
conn.Execute ("update 用户 set "&cwnl&"="&cwnl&"+"&int(clng(zt(4))/10)&",cw='"&temp&"' where 姓名='" & aqjh_name &"'")
conn.Execute ("update 用户 set 体力=体力-"&clng(zt(4))&" where 姓名='" & to1 &"'")
rs.open "select 体力 from 用户 where 姓名='" & to1 &"'",conn,2,2
if rs("体力")<-100 then
	call boot(to1,"发招，操作者："&aqjh_name&",["&zt(0)&"]让宠物打死"&fn1)
