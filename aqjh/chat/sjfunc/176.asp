...
rs.open "select w6,等级,体力,职业,法力 FROM 用户 WHERE 姓名='" & aqjh_name &"'" ,conn,2,2
fn1=abs(fn1)
...
Set rs=Server.CreateObject("ADODB.RecordSet")
conn.open Application("aqjh_usermdb")
rs.open "select 体力,法力,w6 FROM 用户 WHERE 姓名='" & aqjh_name &"'" ,conn
if rs("体力")>5000000 then
lbsw=aqjh_name & " 你的体力够高的了，还要补吗？嘿嘿嘿.(你个人体力大于5000000)！"
else
duyao=abate(rs("w6"),"蓝玛瑙",1)
conn.execute "update 用户 set w6='"&duyao&"',体力=体力+"&fn1&",法力=法力-2000 where 姓名='" & aqjh_name &"'"
lbsw="传闻<font color=red size=2>" & aqjh_name & "</font> 使用了发光的<font color=blue>蓝玛瑙</font>体力狂增"&fn1&"点."
end if
rs.close
set rs=nothing	
conn.close
