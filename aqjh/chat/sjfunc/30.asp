...
rs.open "select 内力,体力,武功,等级,武功加,操作时间 FROM 用户 WHERE 姓名='" & aqjh_name &"'",conn,2,2
...
if rs("武功")<rs("等级")*aqjh_wgsx+3800+rs("武功加") then
	conn.execute "update 用户 set 武功=武功+430,体力=体力-2000,内力=内力-1000,操作时间=now() where 姓名='" & aqjh_name &"'"
...
if rs("武功加")<=rs("等级")*1400 then
	conn.execute "update 用户 set 武功加=武功加+9,体力=体力-2000,内力=内力-1000,操作时间=now() where 姓名='" & aqjh_name &"'"
