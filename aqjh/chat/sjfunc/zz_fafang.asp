...
if aqjh_jhdj<30 then
	Response.Write "<script language=JavaScript>{alert('提示：领取金币需要30级才可以操作！');}</script>"
	Response.End
end if
...
if application("aqjh_zzffjl")<>"" then
	...
	for i=0 to jsq
		...
		if fsz(0)=aqjh_name and clng(fsz(1))=hour(time()) then
			erase fsz
			erase ff
			Response.Write "<script language=JavaScript>{alert('提示：你已经领过金币了。');}</script>"
			Response.End
		end if
...
application.unlock()
fffff="<marquee width=100% scrollamount=8><font color=green>【领取救济】</font><font color=red>"&aqjh_name&"</font>领到了站长发放的<font color=red><b>"&ffsl&"金币</b>，站长支持在线真正玩游戏，不要开小差呀</font>！</marquee>"&tt
conn.execute "update 用户 set 金币=金币+"&ffsl&",操作时间=now()+(31/86400) where 姓名='"&aqjh_name&"'"
conn.close
set conn=nothing
says=fffff
