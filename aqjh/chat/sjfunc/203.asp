...
if chatinfo(5)<>0 then
	Response.Write "<script language=JavaScript>{alert('提示：在["&chatinfo(0)&"]房间不可以执行！');}</script>"
	Response.End
end if
if aqjh_jhdj<=jhdj_aw then
	Response.Write "<script language=JavaScript>{alert('提示：执行需要["&jhdj_aw&"]级才可以操作！');}</script>"
	Response.End
end if
...
i=instr(says,"$")
fnn1=mid(says,i+1)
says="<font color=green>【执行】<font color=" & saycolor & ">"+facrw(mid(says,i+1),towho)+"</font>"
call chatsay("发招",towhoway,towho,saycolor,addwordcolor,addsays,says)

'执行
function facrw(fn1,to1)
...
rs.close
rs.open "select 门派,保护,杀人数,grade,w10,死亡时间,杀人时间 from 用户 where 姓名='" & aqjh_name &"'",conn,2,2
if rs("门派")="出家" and aqjh_grade<10 then
	rs.close
	set rs=nothing	
	conn.close
...
if sj<30 and rs("grade")<10 then
	...
	conn.close
	set conn=nothing
	Response.Write "<script language=JavaScript>{alert('提示：你刚刚杀了人了，等30分钟吧');}</script>"
	Response.End
end if
...
'删除自己的魔宝
duyao=abate(rs("w10"),zswupin,wusl)
...
if rs("体力")<-100 then
	...
	over="##学艺不精，魔宝伤了自己，啊的一声，倒下了…"
	call boot(aqjh_name,"魔宝，不小心伤了自己！")
end if
rs.close
rs.open "select 姓名,体力,宝物 from 用户 where 姓名='" & to1 &"'",conn,2,2
to1=rs("姓名")
if rs("体力")>-100 then
	facrw="##<bgsound src=wav/XIXING.wav loop=1>向%%下["&zswupin&"]共:"&wusl&"个,使%%的内力减:<font color=red>-" & nl & "</font>体力减:<font color=red>-" & tl & "</font>!自己却消耗体力:<font color=red>-"& int(nl/4 ) &"</font>点!%%头晕晕的<img src='READONLY/open33.gif' width='50' height='40'>"&over
	rs.close
	set rs=nothing
	conn.close
	set conn=nothing
...
facrw="##<bgsound src=wav/XIXING.wav loop=1>向%%执行了["&zswupin&"]共:"&wusl&"个,使%%的内力::<font color=red>-" & nl & "</font>体力::<font color=red>-" & tl & "</font>," & to1 & "咽气前说道“查出凶手,为我报仇……”"&over
conn.execute "update 用户 set 状态='死',事件原因='"&aqjh_name&"|暗器:"&zswupin&"' where 姓名='" & to1 &"'"
'记录
...
conn.close
set conn=nothing
call boot(to1,"法器，操作者："&aqjh_name&","&zswupin)
end function
%>