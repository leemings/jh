...
rs.open "select 法力,w6,等级,体力,职业 FROM 用户 WHERE 姓名='" & aqjh_name &"'" ,conn,2,2
fn1=abs(fn1)
...
Set rs=Server.CreateObject("ADODB.RecordSet")
conn.open Application("aqjh_usermdb")
rs.open "select w6,存款,法力 FROM 用户 WHERE 姓名='" & aqjh_name &"'" ,conn
if rs("存款")>400000000 then
ycs=aqjh_name & " 你还真贪耶！你这人真是贪心呀！...嘿嘿嘿.(你个人银两大于四亿)！"
else
duyao=abate(rs("w6"),"摇钱树",1)
conn.execute "update 用户 set w6='"&duyao&"',法力=法力-1000,存款=存款+"&fn1&" where 姓名='" & aqjh_name &"'"
ycs="恭喜恭喜,<font color=red size=2>" & aqjh_name & "</font>运气真好，不知道从那里找到了棵<font color=red size=2>摇钱树</font> ，只一摇就得了"&fn1&"两银子。"
