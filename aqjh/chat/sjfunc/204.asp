...
<%
if aqjh_jhdj<=jhdj_ai then
	Response.Write "<script language=JavaScript>{alert('提示：发射需要["&jhdj_ai&"]级才可以操作！');}</script>"
	Response.End
end if
...
i=instr(says,"$")
fnn1=mid(says,i+1)
says="<font color=green>【发射】<font color=" & saycolor & ">"+facr(mid(says,i+1),towho)+"</font>"
call chatsay("发招",towhoway,towho,saycolor,addwordcolor,addsays,says)

'发射
function facr(fn1,to1)
...
rs.close
rs.open "select 门派,保护,杀人数,grade,w9,死亡时间 from 用户 where 姓名='" & aqjh_name &"'",conn,2,2
if rs("门派")="出家" and aqjh_grade<10 then
	rs.close
	set rs=nothing	
	conn.close
...
'删除自己的魔宝
duyao=abate(rs("w9"),zswupin,wusl)
conn.execute "update 用户 set w9='"&duyao&"' where  姓名='"&aqjh_name&"'"
rs.close
'取出魔宝杀伤力
...
to1=rs("姓名")
if rs("体力")>-100 then
	facr="##<bgsound src=wav/XIXING.wav loop=1>向%%下["&zswupin&"]共:"&wusl&"个,使%%的内力减:<font color=red>-" & nl & "</font>体力减:<font color=red>-" & tl & "</font>!自己却消耗体力:<font color=red>-"& int(nl/4 ) &"</font>点!%%头晕晕的<img src='READONLY/open33.gif' width='50' height='40'>"&over
	rs.close
	set rs=nothing
	conn.close
	set conn=nothing
...
facr="##<bgsound src=wav/XIXING.wav loop=1>向%%发射了["&zswupin&"]共:"&wusl&"个,使%%的内力::<font color=red>-" & nl & "</font>体力::<font color=red>-" & tl & "</font>," & to1 & "咽气前说道“查出凶手,为我报仇……”"&over
...
conn.close
set conn=nothing
call boot(to1,"魔宝，操作者："&aqjh_name&","&zswupin)
end function
%>
b,a,c,e,d) values ('" & to1 & "',now(),'" & aqjh_name & "','"&zswupin&"','人命')"
rs.close
set rs=nothing	
conn.close
set conn=nothing
