...
if aqjh_jhdj<18 then
	Response.Write "<script language=JavaScript>{alert('提示：领取属性需要大于18级才可以操作！');}</script>"
	Response.End
end if
...
if nowsj>=10 then
	...
	Application("aqjh_zzfafang")=""
        Application.UnLock
	Response.Write "<script language=JavaScript>{alert('提示：站长发放有效时间10秒，过期作废。');}</script>"
	Response.End
end if
name=aqjh_name
...
if application("aqjh_zzffjl")<>"" then
	...
	for i=0 to jsq
		...
		if fsz(0)=sjjh_name and clng(fsz(1))=hour(time()) then
			erase fsz
			erase ff
			Response.Write "<script language=JavaScript>{alert('提示：你已经领过属性了。');}</script>"
			Response.End
		end if
		if clng(fsz(1))=hour(time()) and fsz(2)=myip then
			erase fsz
			erase ff
			Response.Write "<script language=JavaScript>{alert('提示：对不起，同一IP只能领取一次属性。');}</script>"
			Response.End
		end if
		erase fsz
	next
...
rs.open "select 操作时间,allvalue,转生,金,木,水,火,土 from 用户 where 姓名='"&aqjh_name&"'",conn,2,2
s=Hour(time())
f=Minute(time())
m=Second(time())
...
application.unlock()
fffff="<marquee width=100% scrollamount=8><font color=green>【属性发放】</font><font color=red>"&aqjh_name&"</font>领到了站长发放的<font color=blue>金木水火土</font>属性各<font color=red><b>10点</b>，供大家练功，真正玩游戏，不要开小差呀，等会还会发！</font>！</marquee>"&tt
conn.execute "update 用户 set 木=木+10,水=水+10,火=火+10,土=土+10,金=金+10,操作时间=now()+(31/86400) where 姓名='"&aqjh_name&"'"
conn.close
set conn=nothing
says=fffff
