...
if aqjh_grade<grade_jc then
	Response.Write "<script language=JavaScript>{alert('提示：解除监禁需要管理等级["&grade_jc&"]才可以操作！');}</script>"
	Response.End
end if
...
rs.open "select 状态 from 用户 where 姓名='"& fn1 &"'",conn
...
conn.execute "update 用户 set 状态='正常',登录=now(),事件原因='"&aqjh_name&" 解除监禁："&fn1&"' where 姓名='" & fn1 & "'"
shifang= "##:<font color=red>官府的人看"& fn1 &"有心改过，表现良好,经大家一至同意给他一次改过自新的机会，将其释放！</font>"
conn.execute "insert into l(b,c,d,a,e) values ('" & aqjh_name & "','" & fn1 & "','解除监禁',now(),'有心改过，原谅他这一次！')"
rs.close
set rs=nothing
conn.close
set conn=nothing
