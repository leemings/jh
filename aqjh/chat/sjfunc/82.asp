...
rs.open "select grade,银两,身份 from 用户 where 姓名='" & aqjh_name &"'",conn,2,2
if rs("grade")>=6 then
	rs.close
	set rs=nothing	
	conn.close
...
if rs("银两")<1000000 then
	...
	conn.close
	set conn=nothing
	Response.Write "<script language=JavaScript>{alert('提示：加入天网需要银两大于100万');}</script>"
	Response.End
end if
conn.execute "update 用户 set 门派='天网',身份='杀手',grade=1,保护=False where 姓名='" & aqjh_name &"'"
rs.close
set rs=nothing
conn.close
set conn=nothing
...
conn.execute "update 用户 set 门派='游侠',身份='弟子',grade=1,保护=True,存款=int(存款/2),银两=int(银两/2) where 姓名='" & aqjh_name &"'"
rs.close
set rs=nothing
conn.close
set conn=nothing
