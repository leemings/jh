...
if rs("知质")<50  then
	Response.Write "<script language=JavaScript>{alert('需50以上的知质才可以！');}</script>"
	rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
...
if rs("种族")<>"神族"  then
	Response.Write "<script language=JavaScript>{alert('此功能为神族才可以使用！');}</script>"
	rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
...
if rs("武功")<rs("等级")*aqjh_nlsx+2000+rs("武功加") then
	conn.execute "update 用户 set 仙术=仙术+1,知质=知质-50,智力=智力-10,操作时间=now() where 姓名='" & aqjh_name &"'"
	dazhuo="##觉的自己身为神仙，不能总是贪图享乐，在天上人间经过仙人指点，提升了自身的仙术<font color=red>+1</font>点!"
	rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
...
if rs("武功加")<=rs("等级")*15000 then
	conn.execute "update 用户 set 仙术=仙术+2,知质=知质-50,智力=智力-10,操作时间=now() where 姓名='" & aqjh_name &"'"
	dazhuo="##苦练腾云驾雾，经过七七四十九天，终有所成，仙术提高<font color=red>2</font>点,为以后的修为打下了基础，真是一个上进的青年!"
else
