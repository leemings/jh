...
i=instr(says,"$")
fnn1=mid(says,i+1)
says="<bgsound src=wav/faqian.wav loop=1><font color=red>【送花消息】<font color=" & saycolor & ">"+songhua(mid(says,i+1),towho)+"</font>"
...
if wusl=0 or wusl>999 and instr(Application("aqjh_user"),aqjh_name)=0 then
	Response.Write "<script language=JavaScript>{alert('提示：物品数量应大于0小于999！');}</script>"
	Response.End 
end if
Set conn=Server.CreateObject("ADODB.CONNECTION")
Set rs=Server.CreateObject("ADODB.RecordSet")
conn.open Application("aqjh_usermdb")
rs.open "select 道德,魅力,w7 FROM 用户 WHERE 姓名='" & aqjh_name &"'",conn,2,2
...
songhua="[##]向{%%}送上<font color=red>"&wusl&"</font>朵<img src='../hcjs/jhjs/images/"&sm&"'>" & zswupin &" ,也不知道人家愿不愿意接收...<input type=button value='接收' onClick=javascript:tongyi"&regjm+1&".disabled=1;tongyi"&regjm&".disabled=1;window.open('xianhua.asp?fromname="& aqjh_name &"&yn=1&toname="&to1&"&huaming="&zswupin&"&wpsl="&wusl&"','d') name=tongyi"&regjm&"><input type=button value='拒收' onClick=javascript:tongyi"&regjm+1&".disabled=1;tongyi"&regjm&".disabled=1;window.open('xianhua.asp?fromname="& aqjh_name &"&yn=0&toname="&to1&"&huaming="&zswupin&"&wpsl="&wusl&"','d') name=tongyi"&regjm+1&">"
end function
%>