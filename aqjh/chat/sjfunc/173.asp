...
if aqjh_name=to1 or to1="大家"  then
 ptz=aqjh_name & "你吃饱了撑着了?!不能向大家或自己使用多情环呀！"
 conn.close
 set rs=nothing
 set conn=nothing
 exit function
...
Set rs=Server.CreateObject("ADODB.RecordSet")
conn.open Application("aqjh_usermdb")
rs.open "select 等级,状态,攻击,防御,门派 FROM 用户 WHERE 姓名='" & to1 &"'",conn
if rs("门派")="出家" and aqjh_grade<10 then
	rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
	Response.Write "<script language=JavaScript>{alert('提示：他是出家人不能操作！');}</script>"
	Response.End
end if
if rs("门派")="官府" and aqjh_grade<10 then
	...
	conn.close
	set conn=nothing
	Response.Write "<script language=JavaScript>{alert('提示：他是官府人员，你对他有什么意见！');}</script>"
	Response.End
end if
if rs("等级")>500 then
...
conn.execute "update 用户 set 攻击=攻击-攻击/10,防御=防御-防御/10  where 姓名='" & to1 &"'"
ptz="<font color=red size=2>" & aqjh_name & "</font>偷偷的向" & to1 & " 挥起了多情环，好在" & to1 & "功力深厚，只是衣服被戳穿了二层。真是惊险!(" & to1 & "攻击、防御下降了十分之一)....</font>"
end if
rs.close
set rs=nothing	
conn.close
