...
i=instr(says,"$")
fnn1=mid(says,i+1)
says="<font color=red>【魔法师·配制令牌】<font color=" & saycolor & ">"+peibashi(mid(says,i+1),towho)+"</font>"
...
rs.open "select 法力,等级,w10,职业 FROM 用户 WHERE 姓名='"&aqjh_name&"'",conn
dj=rs("等级")
fla=rs("法力")
if rs("法力")<75000 then
	rs.close
...
if isnull(duyao) or duyao=abate(rs("w10"),"红钻石",1) or duyao=abate(rs("w10"),"绿钻石",1) or duyao=abate(rs("w10"),"蓝钻石",1) then 
	...
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('提示：三个钻石缺一不可看清楚在来!');</script>"
	response.end
end if
...
conn.execute "update 用户 set  w10='"&duyao&"' where 姓名='"&aqjh_name&"'"
peibashi=aqjh_name & "<bgsound src=wav/m2.mid loop=1>取出红、绿、蓝三种钻石，三种钻石结合在一起，一道光芒升起，<img src='img/look52.gif'>三种宝石将会化成武林圣火令<img src='img/e5.jpg' width='150' height='80'><br></font>" & aqjh_name & "这武林圣火令--他可号令天下.里面收藏了武林绝学有乾坤大挪移..七伤拳..九阳神功..寒冰绵掌等等..."
