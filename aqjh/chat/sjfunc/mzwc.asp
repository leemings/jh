...
rs.open "select 金币,任务,银两,体力,法力,知质,会员金卡,w6,魔力,金,木,水,火,土,操作时间 from 用户 where 姓名='"&aqjh_name&"'",conn
...
if aqjh_jhdj<90 then
	...
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('江湖等级需要超过90级才可提交任务！');window.close();</script>"
	Response.End
end if
...
if rs("木")<500 then
	...
	conn.close
	set conn=nothing
 	Response.Write "<script language=JavaScript>{alert('你没有500木属性无法完成任务！');location.href = 'rwbz.asp';}</script>"
	response.end
end if
input=request.form("input")
if rs("金")<500 then
	...
	conn.close
	set conn=nothing
 	Response.Write "<script language=JavaScript>{alert('你没有500金属性无法完成任务！');location.href = 'rwbz.asp';}</script>"
	response.end
end if
input=request.form("input")
if rs("魔力")<5 then
	...
	conn.close
	set conn=nothing
 	Response.Write "<script language=JavaScript>{alert('你是魔族的也！怎么连魔力都没有呢！~需要5点，快去修炼吧！');location.href = 'rwbz.asp';}</script>"
	response.end
end if
input=request.form("input")
if rs("水")<500 then
	...
	conn.close
	set conn=nothing
 	Response.Write "<script language=JavaScript>{alert('你没有500水属性无法完成任务！');location.href = 'rwbz.asp';}</script>"
	response.end
end if
input=request.form("input")
if rs("火")<500 then
	...
	conn.close
	set conn=nothing
 	Response.Write "<script language=JavaScript>{alert('你没有500火属性无法完成任务！');location.href = 'rwbz.asp';}</script>"
	response.end
end if
input=request.form("input")
if rs("土")<500 then
	...
	conn.close
	set conn=nothing
 	Response.Write "<script language=JavaScript>{alert('你没有500土属性无法完成任务！');location.href = 'rwbz.asp';}</script>"
	response.end
end if
if trim(rs("任务"))<>"魔族任务" then
	rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
	Response.Write "<script Language=Javascript>alert('提示：您的任务不是魔族任务，所以您不能进行魔族任务操作！！');window.close();</script>"
	response.end
end if
...
conn.execute "update 用户 set 木=木-500 where 姓名='"& aqjh_name &"'"
conn.execute "update 用户 set 金=金-500 where 姓名='"& aqjh_name &"'"
conn.execute "update 用户 set 水=水-500 where 姓名='"& aqjh_name &"'"
conn.execute "update 用户 set 火=火-500 where 姓名='"& aqjh_name &"'"
conn.execute "update 用户 set 土=土-500 where 姓名='"& aqjh_name &"'"
conn.execute "update 用户 set 魔力=魔力-5 where 姓名='"& aqjh_name &"'"
randomize timer
r=int(rnd*10)+1
jinbi=0
jinka=0
...
case 1
	...
	erase js
case 2
	mess=""& aqjh_name &"完成任务武功提高1200，攻击提高500"
	sql="update 用户 set 操作时间=now(),武功=武功+1200,攻击=攻击+500 where 姓名='"&aqjh_name&"'"
case 3
	mess=""& aqjh_name &"完成任务武功提高1000"
	sql="update 用户 set 操作时间=now(),武功=武功+1000 where 姓名='"&aqjh_name&"'"
case 4
	mess=""& aqjh_name &"完成任务武功+1300"
	sql="update 用户 set 操作时间=now(),武功=武功+1300 where 姓名='"&aqjh_name&"'"
case 5
	mess=""& aqjh_name &"完成任务武功+2000"
	sql="update 用户 set 操作时间=now(),武功=武功+2000 where 姓名='"&aqjh_name&"'"
case 6
	mess=""& aqjh_name &"完成任务攻击+120"
	sql="update 用户 set 操作时间=now(),攻击=攻击+120 where 姓名='"&aqjh_name&"'"
	case 7
	mess=""& aqjh_name &"完成任务攻击+150"
	sql="update 用户 set 操作时间=now(),攻击=攻击+150 where 姓名='"&aqjh_name&"'"
	case 8
	mess=""& aqjh_name &"完成任务攻击+180"
	sql="update 用户 set 操作时间=now(),攻击=攻击+180 where 姓名='"&aqjh_name&"'"
	case 9
	mess=""& aqjh_name &"完成任务增加江湖存点增加16660，有可能升级的哦！"
	sql="update 用户 set 操作时间=now(),allvalue=allvalue+16660 where 姓名='"&aqjh_name&"'"
        case 10
        rs.close
