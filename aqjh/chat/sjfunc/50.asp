...
if yinjindian<>"金" and yinjindian<>"银" and yinjindian<>"点" and yinjindian<>"法" and yinjindian<>"轻" then
	Response.Write "<script language=JavaScript>{alert('选择有误：银、金、点、法、轻！');}</script>"
	Response.End 
end if
select case yacz
	case "大"
...
case else
	...
	if (yinliang<10 or yinliang>200) and (yinjindian="点" or yinjindian="金" or yinjindian="法" or yinjindian="轻") then
...
case "银"
	...
	rs.close
	rs.open "select top 1 * FROM g WHERE c='银' and a='"& aqjh_name &"' and f=true",conn,2,2
	if not(rs.eof) or not(rs.bof) then
		if rs("b")="庄家" then
			temp=aqjh_name&"你现在是[银局]庄家，你要搞什么呀!"
		else
			temp=aqjh_name&"你在[银局]为闲压["&rs("e")&"]"&rs("d")&"等着开局吧！"
		end if
		rs.close
		set rs=nothing	
		conn.close
	...
	if 	tempdu=0 then
	id=rs("id")
	conn.execute "update 用户 set 银两=银两-"&yinliang&" where 姓名='" & aqjh_name &"'"
	conn.execute "update g set a='"&aqjh_name&"',f=true,d="&yinliang&",e='"&yacz&"' where id="&id&" and f=false"
	rs.close
	end if
...
'开局了
if durs>=10 then
	sj=DateDiff("s",Application("aqjh_du"),now())
	if sj<5 then
		s=5-sj
...
case "点"
		randomize(timer())
		m2 = Int(6 * Rnd + 1)
		rs.open "select allvalue,会员等级 FROM 用户 WHERE 姓名='" & aqjh_name &"'",conn
		select case rs("会员等级")
		case 1
			hydian=31250
		case 2
	...
	rs.close
	rs.open "select top 1 * FROM g WHERE c='点' and a='"& aqjh_name &"' and f=true",conn
	if not(rs.eof) or not(rs.bof) then
		if rs("b")="庄家" then
	...
	if tempdu=0 then
	id=rs("id")
	conn.execute "update 用户 set allvalue=allvalue-"&yinliang&" where 姓名='" & aqjh_name &"'"
	conn.execute "update g set a='"&aqjh_name&"',f=true,d="&yinliang&",e='"&yacz&"' where id="&id&" and f=false"
	rs.close
	end if
...
case "法"
	randomize(timer())
	m2 = Int(6 * Rnd + 1)
		rs.open "select 法力 FROM 用户 WHERE 姓名='" & aqjh_name &"'",conn
		if rs("法力")<yinliang then
			rs.close
			set rs=nothing	
			conn.close
			set conn=nothing
		...
		rs.close
	rs.open "select top 1 a FROM g WHERE c='法' and b='庄家' and f=true",conn
	if rs.eof or rs.bof then
		rs.close
		set rs=nothing	
		conn.close
		set conn=nothing
		Response.Write "<script language=JavaScript>{alert('现在[法力局]没有庄家，不能在线赌博！！');}</script>"
		Response.End 		
	end if
	rs.close
	rs.open "select top 1 * FROM g WHERE c='法' and a='"& aqjh_name &"' and f=true",conn,2,2
	if not(rs.eof) or not(rs.bof) then
		if rs("b")="庄家" then
			temp=aqjh_name&"你现在是[法力局]庄家，你要搞什么呀!"
		else
			temp=aqjh_name&"你在[法力局]为闲压["&rs("e")&"]"&rs("d")&"等着开局吧！"
		end if
		rs.close
		set rs=nothing	
		conn.close
	...
	rs.close
	rs.open "select top 1 id,a FROM g WHERE c='法' and b='玩家' and f=false",conn,2,2
	tempdu=0
	if rs.eof or rs.bof then
		Response.Write "<script language=JavaScript>{alert('提示：在法力局里现在没有空位正在开局!');}</script>"
		tempdu=1
		rs.close
	end if
	if 	tempdu=0 then
	id=rs("id")
	conn.execute "update 用户 set 法力=法力-"&yinliang&" where 姓名='" & aqjh_name &"'"
	conn.execute "update g set a='"&aqjh_name&"',f=true,d="&yinliang&",e='"&yacz&"' where id="&id&" and f=false"
	rs.close
	end if
	tmprs=conn.execute("Select count(*) As 数量 from g where c='法' and e='大' and f=true")
	dars=tmprs("数量")
	tmprs=conn.execute("Select count(*) As 数量 from g where c='法' and e='小' and f=true")
	xiaors=tmprs("数量")
	tmprs=conn.execute("Select count(*) As 数量 from g where c='法' and e='单' and f=true")
	danrs=tmprs("数量")
	tmprs=conn.execute("Select count(*) As 数量 from g where c='法' and e='双' and f=true")
	shuangrs=tmprs("数量")
	tmprs=conn.execute("Select count(*) As 数量 from g where c='法' and d<>0 and f=true")
	durs=tmprs("数量")
...
if durs>=10 then
	...
	sjdubo=m1+m2+m3
'查找庄家
rs.open "select * FROM g WHERE c='法' and b='庄家' and f=true",conn,2,2
zhuangjia=rs("a")
rs.close

'豹子处理
if m1=m2 and m2=m3 and m3=m1 then
	rs.open "select * FROM g WHERE c='法' and d<>0 and b<>'庄家' and f=true",conn,2,2
	yingyin=0
	do while not rs.bof and not rs.eof
	yingyin=yingyin+rs("d")
	rs.movenext
	...
	rs.close
	'给庄加钱
	conn.execute "update 用户 set 体力=体力+10000,法力=法力+"&yingyin&" where 姓名='"& zhuangjia &"'"
	'清除所有赌博状态
	conn.execute "update g set f=false where f=true and c='法'"
	xiazhu="[法力局]庄家开：<img src='../jhimg/"&m1&".gif'><img src='../jhimg/"&m2&".gif'><img src='../jhimg/"&m3&".gif'>计："& sjdubo &"点!庄家开出豹子……通杀！庄家："&zhuangjia&"收入："&yingyin&"点！气势如宏，体力+1万点！"
		set rs=nothing
		conn.close
		set conn=nothing
		exit function
...
if sjdubo/2=int(sjdubo/2) then
	danshuang="<img src='../jhimg/shuang.gif'>"
	rs.open "select * FROM g WHERE c='法' and e='双' and f=true",conn,2,2
	do while not rs.bof and not rs.eof
		shuangyinliang=shuangyinliang+rs("d")
		shuangname=shuangname&rs("a")&" "
		conn.execute "update 用户 set 法力=法力+"&rs("d")*2&",体力=体力+500,内力=内力+500 where 姓名='"& rs("a") &"'"
		conn.execute "update 用户 set 法力=法力-"& rs("d") &" where 姓名='"& zhuangjia &"'"	
	rs.movenext
	loop
	rs.close
	'清除所有赌博状态
	conn.execute "update g set f=false where f=true and c='法' and e='双'"
end if
if sjdubo/2<>int(sjdubo/2) then
	danshuang="<img src='../jhimg/dan.gif'>"
	rs.open "select * FROM g WHERE c='法' and e='单' and f=true",conn
	do while not rs.bof and not rs.eof
		danyinliang=danyinliang+rs("d")
		danname=danname&rs("e")&" "
		conn.execute "update 用户 set 法力=法力+"&rs("d")*2&",体力=体力+500,内力=内力+500 where 姓名='"& rs("a") &"'"
	...
	loop
	rs.close
	conn.execute "update g set f=false where f=true and c='轻' and e='单'"
end if

'对开大小处理
if sjdubo<=10 then
	daxiao="<img src='../jhimg/xiao.gif'>"
	rs.open "select * FROM g WHERE c='法' and e='小' and f=true",conn,2,2
	do while not rs.bof and not rs.eof
		xiaoyinliang=xiaoyinliang+rs("d")
		xiaoname=xiaoname&rs("a")&" "
		conn.execute "update 用户 set 法力=法力+"&rs("d")*2&",体力=体力+500,内力=内力+500 where 姓名='"& rs("a") &"'"
	...
	loop
	rs.close
	conn.execute "update g set f=false where f=true and c='法' and e='小'"
end if
if sjdubo>10 then
	daxiao="<img src='../jhimg/da.gif'>"
	rs.open "select * FROM g WHERE c='法' and e='大' and f=true",conn,2,2
	do while not rs.bof and not rs.eof
		dayinliang=dayinliang+rs("d")
		daname=daname&rs("a")&" "
		conn.execute "update 用户 set 法力=法力+"&rs("d")*2&",体力=体力+500,内力=内力+500 where 姓名='"& rs("a") &"'"
	...
	loop
	rs.close
	conn.execute "update g set f=false where f=true and c='法' and e='大'"
end if
...
'对剩下输的用户法力
	...
	loop
	rs.close
	conn.execute "update 用户 set 法力=法力+"&yingyin&" where 姓名='"& zhuangjia &"'"
	conn.execute "update g set f=false where f=true and c='法'"
	zong=yingyin+shuangyinliang+danyinliang+xiaoyinliang+dayinliang
	pei=shuangyinliang+danyinliang+xiaoyinliang+dayinliang
	duboname=shuangname&danname&xiaoname&daname
	xiazhu="[法力局]庄家开：<img src='../jhimg/"&m1&".gif'><img src='../jhimg/"&m2&".gif'><img src='../jhimg/"&m3&".gif'>计：<font color=blue>"& sjdubo &"</font>点!"&danshuang&daxiao&"总下注：<font color=blue>"&zong&"</font>点，庄家：["&zhuangjia&"]收入：<font color=blue>"&yingyin &"</font>点,赔出：<font color=blue>"&pei&"</font>点，合计：<font color=blue>"&yingyin-pei&"</font>点,共有：<font color=red>"&duboname&"</font>玩家幸运押中,所有押中玩家内力、体力各加+500！"
	set rs=nothing
	conn.close
	set conn=nothing
	exit function
...
xiazhu="[法力局]##从自己的法力宝瓶子里拿出:"& yinliang &"点，我买"& duboimg &"！，一定中的！现在下注如下：押大：<font color=blue>"& dars &"</font>个，押小:<font color=blue>"& xiaors &"</font>个,押单：<font color=blue>"&danrs&"</font>个,押双:<font color=blue>"&shuangrs&"</font>个！还差:<font color=blue>"&(10-durs)&"</font>个开局！"
	set rs=nothing	
	conn.close
	set conn=nothing
...
case "轻"
	randomize(timer())
	m2 = Int(6 * Rnd + 1)
		rs.open "select 轻功 FROM 用户 WHERE 姓名='" & aqjh_name &"'",conn
		if rs("轻功")<yinliang then
			rs.close
			set rs=nothing	
			conn.close
			set conn=nothing
			Response.Write "<script language=JavaScript>{alert('你有这么多的轻功吗？！');}</script>"
			Response.End 		
		end if
		rs.close
	rs.open "select top 1 a FROM g WHERE c='轻' and b='庄家' and f=true",conn
	if rs.eof or rs.bof then
		rs.close
		set rs=nothing	
		conn.close
		set conn=nothing
		Response.Write "<script language=JavaScript>{alert('现在[轻功局]没有庄家，不能在线赌博！！');}</script>"
		Response.End 		
	end if
	rs.close
	rs.open "select top 1 * FROM g WHERE c='轻' and a='"& aqjh_name &"' and f=true",conn,2,2
	if not(rs.eof) or not(rs.bof) then
		if rs("b")="庄家" then
			temp=aqjh_name&"你现在是[轻功局]庄家，你要搞什么呀!"
		else
			temp=aqjh_name&"你在[轻功局]为闲压["&rs("e")&"]"&rs("d")&"等着开局吧！"
		end if
		rs.close
		set rs=nothing	
		conn.close
	...
	rs.close
	rs.open "select top 1 id,a FROM g WHERE c='轻' and b='玩家' and f=false",conn,2,2
	tempdu=0
	if rs.eof or rs.bof then
		Response.Write "<script language=JavaScript>{alert('提示：在轻功局里现在没有空位正在开局!');}</script>"
		tempdu=1
		rs.close
	end if
	if 	tempdu=0 then
	id=rs("id")
	conn.execute "update 用户 set 轻功=轻功-"&yinliang&" where 姓名='" & aqjh_name &"'"
	conn.execute "update g set a='"&aqjh_name&"',f=true,d="&yinliang&",e='"&yacz&"' where id="&id&" and f=false"
	rs.close
	end if
	tmprs=conn.execute("Select count(*) As 数量 from g where c='轻' and e='大' and f=true")
	dars=tmprs("数量")
	tmprs=conn.execute("Select count(*) As 数量 from g where c='轻' and e='小' and f=true")
	xiaors=tmprs("数量")
	tmprs=conn.execute("Select count(*) As 数量 from g where c='轻' and e='单' and f=true")
	danrs=tmprs("数量")
	tmprs=conn.execute("Select count(*) As 数量 from g where c='轻' and e='双' and f=true")
	shuangrs=tmprs("数量")
	tmprs=conn.execute("Select count(*) As 数量 from g where c='轻' and d<>0 and f=true")
	durs=tmprs("数量")
...
if durs>=10 then
	...
	sjdubo=m1+m2+m3
'查找庄家
rs.open "select * FROM g WHERE c='轻' and b='庄家' and f=true",conn,2,2
zhuangjia=rs("a")
rs.close

'豹子处理
if m1=m2 and m2=m3 and m3=m1 then
	rs.open "select * FROM g WHERE c='轻' and d<>0 and b<>'庄家' and f=true",conn,2,2
	yingyin=0
	do while not rs.bof and not rs.eof
	yingyin=yingyin+rs("d")
	rs.movenext
	...
	rs.close
	'给庄加钱
	conn.execute "update 用户 set 体力=体力+10000,轻功=轻功+"&yingyin&" where 姓名='"& zhuangjia &"'"
	'清除所有赌博状态
	conn.execute "update g set f=false where f=true and c='轻'"
	xiazhu="[轻功局]庄家开：<img src='../jhimg/"&m1&".gif'><img src='../jhimg/"&m2&".gif'><img src='../jhimg/"&m3&".gif'>计："& sjdubo &"点!庄家开出豹子……通杀！庄家："&zhuangjia&"收入："&yingyin&"点！气势如宏，体力+1万点！"
		set rs=nothing
		conn.close
		set conn=nothing
		exit function
...
if sjdubo/2=int(sjdubo/2) then
	danshuang="<img src='../jhimg/shuang.gif'>"
	rs.open "select * FROM g WHERE c='轻' and e='双' and f=true",conn,2,2
	do while not rs.bof and not rs.eof
		shuangyinliang=shuangyinliang+rs("d")
		shuangname=shuangname&rs("a")&" "
		conn.execute "update 用户 set 轻功=轻功+"&rs("d")*2&",体力=体力+500,内力=内力+500 where 姓名='"& rs("a") &"'"
		conn.execute "update 用户 set 轻功=轻功-"& rs("d") &" where 姓名='"& zhuangjia &"'"	
	rs.movenext
	loop
	rs.close
	'清除所有赌博状态
	conn.execute "update g set f=false where f=true and c='轻' and e='双'"
end if
if sjdubo/2<>int(sjdubo/2) then
	danshuang="<img src='../jhimg/dan.gif'>"
	rs.open "select * FROM g WHERE c='轻' and e='单' and f=true",conn
	do while not rs.bof and not rs.eof
		danyinliang=danyinliang+rs("d")
		danname=danname&rs("e")&" "
		conn.execute "update 用户 set 轻功=轻功+"&rs("d")*2&",体力=体力+500,内力=内力+500 where 姓名='"& rs("a") &"'"
...
if sjdubo<=10 then
	daxiao="<img src='../jhimg/xiao.gif'>"
	rs.open "select * FROM g WHERE c='轻' and e='小' and f=true",conn,2,2
	do while not rs.bof and not rs.eof
		xiaoyinliang=xiaoyinliang+rs("d")
		xiaoname=xiaoname&rs("a")&" "
		conn.execute "update 用户 set 轻轼=轻功+"&rs("d")*2&",体力=体力+500,内力=内力+500 where 姓名='"& rs("a") &"'"
		conn.execute "update 用户 set 轻功=轻功-"& rs("d") &" where 姓名='"& zhuangjia &"'"
	rs.movenext	
	loop
	rs.close
	conn.execute "update g set f=false where f=true and c='轻' and e='小'"
end if
if sjdubo>10 then
	daxiao="<img src='../jhimg/da.gif'>"
	rs.open "select * FROM g WHERE c='轻' and e='大' and f=true",conn,2,2
	do while not rs.bof and not rs.eof
		dayinliang=dayinliang+rs("d")
		daname=daname&rs("a")&" "
		conn.execute "update 用户 set 轻功=轻功+"&rs("d")*2&",体力=体力+500,内力=内力+500 where 姓名='"& rs("a") &"'"
	...
	loop
	rs.close
	conn.execute "update g set f=false where f=true and c='轻' and e='大'"
end if
...
'对剩下输的用户轻功
	...
	loop
	rs.close
	conn.execute "update 用户 set 轻功=轻功+"&yingyin&" where 姓名='"& zhuangjia &"'"
	conn.execute "update g set f=false where f=true and c='轻'"
	zong=yingyin+shuangyinliang+danyinliang+xiaoyinliang+dayinliang
	pei=shuangyinliang+danyinliang+xiaoyinliang+dayinliang
	duboname=shuangname&danname&xiaoname&daname
	xiazhu="[轻功局]庄家开：<img src='../jhimg/"&m1&".gif'><img src='../jhimg/"&m2&".gif'><img src='../jhimg/"&m3&".gif'>计：<font color=blue>"& sjdubo &"</font>点!"&danshuang&daxiao&"总下注：<font color=blue>"&zong&"</font>点，庄家：["&zhuangjia&"]收入：<font color=blue>"&yingyin &"</font>点,赔出：<font color=blue>"&pei&"</font>，合计：<font color=blue>"&yingyin-pei&"</font>点,共有：<font color=red>"&duboname&"</font>玩家幸运押中,所有押中玩家内力、体力各加+500！"
	set rs=nothing
	conn.close
	set conn=nothing
	exit function
...
xiazhu="[轻功局]##从自己的轻功宝瓶子里拿出:"& yinliang &"点，我买"& duboimg &"！，一定中的！现在下注如下：押大：<font color=blue>"& dars &"</font>个，押小:<font color=blue>"& xiaors &"</font>个,押单：<font color=blue>"&danrs&"</font>个,押双:<font color=blue>"&shuangrs&"</font>个！还差:<font color=blue>"&(10-durs)&"</font>个开局！"
	set rs=nothing	
	conn.close
	set conn=nothing
