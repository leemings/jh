...
rs.open "select 体力,内力,法力 from 用户 where 姓名='" & aqjh_name &"'",conn,2,2
if rs("体力")<1000 then
	rs.close
	set rs=nothing	
	conn.close
...
conn.execute "update 用户 set 体力=体力-1000,内力=内力-1000,法力=法力-"&fontx&" where 姓名='" & aqjh_name &"'"
rs.close
set rs=nothing
conn.close
set conn=nothing
...
conn.execute "update 用户 set 体力=体力-1000,内力=内力-1000,法力=法力-500 where 姓名='" & aqjh_name &"'"
rs.close
set rs=nothing
conn.close
set conn=nothing
