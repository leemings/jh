...
if rs("存款")<money then
	...
	conn.close
	set conn=nothing
	Response.Write "<script language=JavaScript>{alert('提示：你钱好象不够！');}</script>"
	Response.End
end if
rs.close
rs.open "select top 1 * FROM g WHERE c='马' and a='"& aqjh_name &"' and f=true",conn,2,2
if not(rs.eof) or not(rs.bof) then
	temp="提示："&aqjh_name&"你买了["&rs("b")&"]号马计："&rs("d")&"两，等着开始吧!"
	rs.close
	set rs=nothing	
	conn.close
	set conn=nothing
...
if 	tempdu=0 then
	id=rs("id")
	conn.execute "update 用户 set 存款=存款-"&money&" where 姓名='" & aqjh_name &"'"
	conn.execute "update g set a='"&aqjh_name&"',f=true,d="&money&",b='"&radiobutton&"' where id="&id&" and f=false"
	rs.close
end if
...
conn.close
set conn=nothing
says="<font color=blue><b>[冰心马场]</b></font>"&aqjh_name&"在江湖马上选了一匹好马<img src=horse/"&radiobutton&".gif>["&radiobutton&"]号，在这匹马上下注:<font color=red>"&money&"</font>两，现在还差<font color=blue>"&(10-dars)&"</font>开赛！"
call chat(says)
Response.End 
