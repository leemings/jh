...
if f<00 or f>25 then
	Response.Write "<script language=JavaScript>{alert('杀人时间为每小时的前25分钟！');window.close();}</script>"
	Response.End 
end if
...
if rs("等级")<25 then
	Response.Write "<script Language=Javascript>alert('提示：你的等级不够[25],不可以使用此招！');</script>"
	myclose()
end if
...
if rs("等级")<50 then
	Response.Write "<script Language=Javascript>alert('提示：他还是新人，你不可攻击他！');</script>"
	myclose()
end if
...
if shengtili<-100 then
	...
	if dgjjzs="破气式" then
		if lgbh=true then
			...
			lgbh=false
			e="看来他要倒大霉了。"
			stunt=aqjh_name & "<bgsound src=wav/bsj.wav loop=1>运足" & nl & "点内力，" & tl & "点体力，对<img src='img/41.gif'><font color=blue>" & to1 & "</font>使用了江湖上久已传的独孤九剑之<font color=008000>【"&dgjjzs&"】</font>，去除了<font color=blue>" & to1 & "</font>的<font color=red>练功保护<font/>。" & e
		else
			conn.execute "update 用户 set 杀人时间=now(),状态='死',银两=0 where 姓名='" & to1 & "'"
			conn.execute "update 用户 set 知质=知质-15,银两=银两+" & yinliang2 & " where 姓名='" & aqjh_name & "'"
			
			e="点，" & to1 & "<bgsound src=wav/daipu.wav loop=1>慢慢的倒了下去……从此江湖上又少了一只大虾," & aqjh_name & "得到" & to1 & "的所有现金" & yinliang2 & "两，" & to1 & "的所有物品从此散落江湖!"
			fn1=dgjjzs
			call boot(to1,"独孤九剑，操作者："&aqjh_name&",["&mp&"]"&fn1)
			conn.execute "insert into l(b,a,c,e,d) values ('" & to1 & "',now(),'" & aqjh_name & "','独孤九剑之"&dgjjzs&"','人命')"
		end if
	else
		conn.execute "update 用户 set 状态='死',银两=0,事件原因='"&aqjh_name&"|独孤九剑之"&dgjjzs&"' where 姓名='" & to1 & "'"
		if tjf=True then
			conn.execute "update 用户 set 银两=0,存款=int(存款/2),道德=0,魅力=0 where 姓名='" & to1 & "'"
		end if
		conn.execute "update 用户 set 知质=知质-15,杀人时间=now(),银两=银两+" & yinliang2 & " where 姓名='" & aqjh_name & "'"
		e="点，" & to1 & "<bgsound src=wav/daipu.wav loop=1>慢慢的倒了下去……从此江湖上又少了一只大虾," & aqjh_name & "得到" & to1 & "的所有现金" & yinliang2 & "两！"
		fn1=dgjjzs
		call boot(to1,"独孤九剑，操作者："&aqjh_name&"，["&mp&"]"&fn1)
		conn.execute "insert into l(b,a,c,e,d) values ('" & to1 & "',now(),'" & aqjh_name & "','独孤九剑之"&dgjjzs&"','人命')"
	end if
...
if stunt="" then
	stunt=aqjh_name & "<bgsound src=wav/bsj.wav loop=1>运足" & nl & "点内力，" & tl & "点体力，对<img src='img/021.gif'><font color=blue>" & to1 & "</font>使用了江湖上久已传的独孤九剑之<font color=008000>【"&dgjjzs&"】</font>，杀伤" & killer & e
end if
rs.close
set rs=nothing
conn.close
