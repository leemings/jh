...
if act<>"" then
	...
	if session("aqjh_grade")<application("aqjh_npcff") and act>1 then
		...
		conn.close
		set conn=nothing
		Response.Write "<script Language=Javascript>alert('提示：" & aqjh_name & "你没有发放NPC的权力!');location.href = 'seenpc.asp?id=" & id & "';</script>"
		Response.end
	end if
	select case act
	case 1'唤醒npc使用的是金币这个与怪物等相相关，每一级怪物就是一个金币大家可以改jbmoney=1来自己改倍数
		...
		if npcsl>=Application("aqjh_maxnpc") Then 
			...
			conn.close
			set conn=nothing
			Response.Write "<script Language=Javascript>alert('提示：" & aqjh_name & "江湖规定江湖npc在线数量最多["& Application("aqjh_maxnpc") &"]个!');location.href = 'seenpc.asp?id=" & id & "';</script>"
			Response.end
		end if
		...
		if InStr(";" & Application("aqjh_npc"), ";" & name & "|")<>0 then
			...
			conn.close
			set conn=nothing
			Response.Write "<script Language=Javascript>alert('提示：此NPC现在已经在线了，你不能进行唤醒操作!');location.href = 'seenpc.asp?id=" & id & "';</script>"
			Response.end
		end if
		zuren=0
		...
		'判断自己的金币数量,如果这个npc是我的，我花一半钱召出来
			...
			if rs("金币")<jbsl then
				...
				conn.close
				set conn=nothing
				Response.Write "<script Language=Javascript>alert('提示：你没有足够的金和币，不能唤醒此NPC!');location.href = 'seenpc.asp?id=" & id & "';</script>"
				Response.end
			end if
                        if rs("职业")<>"魔法师" then
				rs.close
				...
				conn.close
				set conn=nothing
				Response.Write "<script Language=Javascript>alert('提示：只有魔法师才能唤醒此NPC!');location.href = 'seenpc.asp?id=" & id & "';</script>"
				Response.end
			end if
			rs("金币")=rs("金币")-jbsl
			rs.update
...
call showchat(says)
	...
	case 2,3
		...
		if act=2 then
			if InStr(";" & Application("aqjh_npc"), ";" & name & "|")<>0 then
			Response.Write "<script Language=Javascript>alert('提示：此NPC已经在线了，不能再放出!');location.href = 'seenpc.asp?id=" & id & "';</script>"
			Response.end
			end if
			...
			Response.Write "<script Language=Javascript>alert('提示：官府每半小时只能发放一次NPC！站长不受限制！');location.href = 'seenpc.asp?id=" & id & "';</script>"
			Response.end
                        end if
...
call showchat(says)
                       ...
                       if InStr(";" & Application("aqjh_npc"), ";" & name & "|")=0 then
				Response.Write "<script Language=Javascript>alert('提示：此NPC不在线你不能踢出他!');location.href = 'seenpc.asp?id=" & id & "';</script>"
				Response.end
			end if
'删除名单
nowinroom=session("nowinroom")
...
call showchat(says)
		...
		Response.Write "<script Language=Javascript>alert('提示："& show &"');location.href = 'seenpc.asp?id=" & id & "';</script>"
		Response.end
	end select
end if
rs.open "Select  * from [npc] where id=" & id,conn,1,1
...
<input type="submit" name="Submit" onclick="javascript:<%If online<>0 Then%>alert('此NPC已经在线不能再唤醒!');<%else%>if(confirm('唤醒此NPC需要金币：<%=(int(sqr(int(rs("n经验")/50)))*jbmoney)%>个，如果你是它的主人只需花一半的钱 \n唤醒你就是他的主人，你确定进行唤醒操作吗？')){location.href='seenpc.asp?id=<%=id%>&act=1';}<%end if%>" value="唤醒NPC">&nbsp;&nbsp;&nbsp;
<%if session("aqjh_grade")>=application("aqjh_npcff") or Session("aqjh_name") = Application("aqjh_user") then%>
<input type="submit" name="Submit2" onclick="javascript:<%If online<>0 Then%>alert('此NPC已经在线不能再发放!');<%else%>location.href='seenpc.asp?id=<%=id%>&act=2';<%end if%>" value="发放NPC">&nbsp;&nbsp;&nbsp;
<input type="submit" name="Submit3" onclick="javascript:<%If online=0 Then%>alert('此NPC并不在线不能再踢出!');<%else%>location.href='seenpc.asp?id=<%=id%>&act=3';<%end if%>" value="踢出NPC">
<%end if%>
</td>
</tr>
<tr>
...
论坛地址：<a href=http://bbs.7758530.com target=_blank><font color=yellow>http://www.7758530.com</font></a><br>联系永不放弃 QQ:51726805</p>
</td>
</tr>
</table>
</body>
