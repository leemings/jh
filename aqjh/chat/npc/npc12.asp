...
if Application("aqjh_klt12")=0 then
	Response.Write "<script Language=Javascript>alert('提示：怪物已经不在了！');</script>"
	Response.end
end if
r=int(abs(clng(Request.QueryString("r"))))
tempjs=int(abs(clng(Application("aqjh_klt12"))))
if tempjs<>r then
	Response.Write "<script Language=Javascript>alert('提示：滚，你想搞什么，按回车键不行了!');</script>"
	response.end
end if
Application.Lock
Application("aqjh_klt12")=0
Application.UnLock
Set conn=Server.CreateObject("ADODB.CONNECTION")
Set rs=Server.CreateObject("ADODB.RecordSet")
conn.open Application("aqjh_usermdb")
conn.execute "update 用户 set 体力=体力-"&Application("aqjh_klt12")&" where 姓名='"&st&"'"
...
conn.execute "insert into l(b,a,c,e,d) values ('" & st & "',now(),'吝啬','体力不足累死','人命')"
	...
	conn.execute "update 用户 set 防御=防御+"& tempjs*30&" where 姓名='" & st & "'"
	kl="<font color=#0000FF>"&st&"</font>一翻苦言,劝说大家不要伤害江湖保护动物,并把梦精灵送回山上，梦精灵帮<font color=#0000FF>"&st&"</font>提高防御"&tempjs*30&"点!"
	end if
	rs.close
set rs=nothing
conn.close
