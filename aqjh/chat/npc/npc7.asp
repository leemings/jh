...
if Application("aqjh_klt7")=0 then
	Response.Write "<script Language=Javascript>alert('提示：怪物已经不在了！');</script>"
	Response.end
end if
tempjs=int(abs(clng(Application("aqjh_klt7"))))
'if tempjs<>r then
	'Response.Write "<script Language=Javascript>alert('提示：滚，你想搞什么，按回车键不行了!');</script>"
	'response.end
'end if
Application.Lock
Application("aqjh_klt7")=0
Application.UnLock
Set conn=Server.CreateObject("ADODB.CONNECTION")
Set rs=Server.CreateObject("ADODB.RecordSet")
conn.open Application("aqjh_usermdb")
conn.execute "update 用户 set 体力=体力-"&Application("aqjh_klt7")&" where 姓名='"&st&"'"
...
conn.execute "insert into l(b,a,c,e,d) values ('" & st & "',now(),'吝啬','体力不足累死','人命')"
	...
	conn.execute "update 用户 set 法力=法力+"& tempjs *50&" where 姓名='" & st & "'"
	kl="<font color=#0000FF>"&st&"</font>历尽千辛万苦，终于将火龙打败，等到法力"&tempjs*50&"点！"
	end if
	rs.close
set rs=nothing
conn.close
