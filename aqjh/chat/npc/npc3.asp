...
if Application("aqjh_klt3")=0 then
	Response.Write "<script Language=Javascript>alert('提示：恐怖飞兽已经不在了！');</script>"
	Response.end
end if
tempjs=int(abs(clng(Application("aqjh_klt3"))))
'if tempjs<>r then
	'Response.Write "<script Language=Javascript>alert('提示：滚，你想搞什么，按回车键不行了!');</script>"
	'response.end
'end if
Application.Lock
Application("aqjh_klt3")=0
Application.UnLock
...
if rs("体力")<0 then
	conn.execute "update 用户 set 状态='死',事件原因='恐怖飞兽|"&fn1&"' where 姓名='"&st&"'"
conn.execute "insert into l(b,a,c,e,d) values ('" & st & "',now(),'吝啬','体力不足累死','人命')"
	kl="<img src='pic/kl.gif'>太不幸了，["&st&"]为了抵御恐怖飞兽袭击，舍身取义，把生命永远的留在了彩虹之下，陪伴他的只有千百年仍在变幻着神奇光芒的传说………"
	Response.Write "<script Language=Javascript>alert('提示：你英勇可嘉，但是体力太弱，丢了小命，去复活吧！没能耐别乱闯！');</script>"
call boot(st,"怪物，操作者：怪物，咆哮！")
else
	conn.execute "update 用户 set 防御=防御+"& tempjs*50&" where 姓名='" & st & "'"
	kl="<font color=#0000FF>"&st&"</font>一招打死了恐怖飞兽，获取恐怖飞兽身上的防御"&tempjs*50&"点！"
	end if
	rs.close
set rs=nothing
conn.close
