...
if Application("aqjh_yb")=0 then
	Response.Write "<script Language=Javascript>alert('提示：你小子来晚了，金卡没有了...');</script>"
	response.end
end if
...
conn.execute "update 用户 set 会员金卡=会员金卡+"& temp &" where 姓名='" & aqjh_name &"'"
yb="今天真是走运，收到站长送的金卡，呵。。。因此：["&name&"]得到金卡:"&temp&"块!"
conn.close
set conn=nothing
