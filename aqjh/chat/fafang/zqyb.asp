...
if Application("aqjh_zqyb")=0 then
	Response.Write "<script Language=Javascript>alert('提示：你小子来晚了，月饼早被人吃掉了！');</script>"
	response.end
end if
...
if rs("体力")<-10000 then
	conn.execute "update 用户 set 状态='死',事件原因='抢月饼|"&fn1&"' where 姓名='" & aqjh_name &"'"
	conn.execute "insert into l(b,a,c,e,d) values ('" & aqjh_name & "',now(),'月饼','抢月饼不成被人踩死','人命')"
	kl="<img src='img/zqyb.gif'>哈哈，["&aqjh_name&"]急着抢月饼，但是不小心，结果被人给踩死了………"
	call boot(aqjh_name,"急着抢月饼，但是不小心，结果被人给踩死了！")
else
dim js(10)
js(0) ="清醒卡"
js(1) ="补血卡"
...
kl="站长送月饼来了["&aqjh_name&"]跑的最快，抢在大家前面把月饼领走了"&aqjh_name&"得到站长送出的[<b><font color=red>"&js(myxy)&"</font></b>]1张……"
end if
rs.close
set rs=nothing
conn.close
