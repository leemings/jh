  ...
  <tr> 
    <td height="30" colspan="7" bgcolor="#FFFFFF"> 
      <p align=center class="p9"><font style="FONT-SIZE: 9pt" color="#FF3333"><marquee><font color="#990099"> 
        <a href="../welcome.asp">返回江湖</a> 欢迎</font><%=name%><font color="#990099">光临房产中心，谁都想有自己的家，这里可以满足您的各种要求：(现在卖出房子只能得到原来的1/5的价格)</font></marquee></font></p>  
  <tr>   
...
%>   
  <tr bgcolor="#FFFFFF"  onmouseout="this.bgColor='#FFFFFF';"onmouseover="this.bgColor='#DFEFFF';">   
    ...
    <td width="19%" class="calen-curr" height="15">   
      <div align="center"><font size="2"><%=rs("户主")%> / </font><font color="ff00ff" size="2"><%=rs("伴侣")%></font></div>  
    </td>  
    <td width="25%" class="calen-curr" height="15"><font size="2">   
      <center>  
        <%if rs("户主")<>name and rs("户主")="无" then%>  
        <a href=fangwu1.asp?id=<%=rs("id")%>><font color="0000ff">购买</font></a>  
        <%end if%>  
        <%if rs("户主")=aqjh_name then%>  
        <a href=fangwu2.asp?id=<%=rs("id")%>><font color=red>卖出</font></a>  
        <%end if%>  
        <%if rs("户主")<>"无" and rs("户主")<>name then%>  
        该房己出售  
        <%end if%>  
      </center>  
      </font></td>  
