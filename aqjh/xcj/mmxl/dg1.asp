...
if rs("门派基金")<1000000 then
		Response.Write "<script language=javascript>alert('抱歉！你对门派贡献金少于100万，不能在此练功！');history.back();</script>"
		rs.close
		set rs=nothing
		conn.close
		set conn=nothing
...
rs.open "select * from 用户 where 姓名='"&aqjh_name&"'",conn
yy=20+gg*2
conn.execute "update 用户 set 体力=体力-10000,内力=内力-10000,银两=银两-100000,allvalue=allvalue+"&yy&",操作时间=now() where 姓名='"&aqjh_name&"'"
%>
<%
rs.close
set rs=nothing
conn.close
set conn=nothing
says="<font color=red>【门派修炼】["&aqjh_name&"]</font><font color=blue>付了10万两白银在门派[<font color=red>"&mp&"</font>]练功处修炼绝世武功，内力减少<font color=red>10000</font>，体力减少<font color=red>10000</font>，总积分上涨<font color=red>"&yy&"</font>点，相信经过他的不断努力，终究会成为一代高手的！</font>"
call showchat(says)
%>
<html>
<head>
